...
$(function(){
	...
	$(".js-confirm-refund-ok").click(function(){
		...
		var refund_records_id = $("#confirm_refund_records_id").val();
		var refund_require_money = $("#refund_require_money").val();
		...
		if(validation()){
			...
			if(parseFloat($("#refund_money_input").val()) != parseFloat($("#refund_require_money").val())){
				html_prompt = '<p>注意:您输入的退款金额与买家本次申请退款金额不一致,可能会导致退款金额有误,请核对好退款信息！</p>';
			}
			...
			$("#countdown_refund_confirm").attr("data-refund_records_id",refund_records_id);
	...
	$("#countdown_refund_confirm").click(function(){
		if(!$(this).hasClass("disabled") && $(this).attr("data-order-id") && $(this).attr("data-order-goods-id") && $(this).attr("data-refund_records_id")){
			orderGoodsConfirmRefund($(this).attr("data-order-id"),$(this).attr("data-order-goods-id"),$(this).attr("data-refund_records_id"),$(this).attr("data-refund-money"),$("#balance_refund").attr("data-refund-balance-money"));
		}
	});
})
/**
...
function orderGoodsRefundMoney(order_goods_id){
	$.ajax({
		...
		success : function(data){
			$("#agree_pay_money").text(parseFloat(data).toFixed(2) + "元");
			$("#refuse_pay_money").text(parseFloat(data).toFixed(2) + "元");
			$("#pay_money").text(parseFloat(data).toFixed(2) + "元").attr("data-pay-money",parseFloat(data).toFixed(2));
...
//退款
function orderGoodsConfirmRefund(order_id,order_goods_id,refund_records_id,refund_money,refund_balance_money){
...
//	console.log(order_id+","+order_goods_id+","+refund_money+","+refund_balance_money);
	...
	$.ajax({
		type : "post",
		url : __URL(ADMINMAIN + "/order/ordergoodsconfirmrefunds"),
		data : {'order_id':order_id,"order_goods_id":order_goods_id,"refund_records_id":refund_records_id, "refund_real_money":refund_money, "refund_balance_money" : refund_balance_money, "refund_way" : $("#refund_way_select").val(), "refund_remark" : $("#refund-remark").val() },
		success : function(data) {
...
function agreeRefund(){
	...
	var refund_records_id = $("#agree_refund_records_id").val();
	$.ajax({
		type : "post",
		url : __URL(ADMINMAIN + "/order/ordergoodsrefundagrees"),
		data : {'order_id':order_id,"order_goods_id":order_goods_id,"refund_records_id":refund_records_id},
		success : function(data) {
			if (data['code'] > 0) {
...
/**
 * 确认退款界面显示
 * refund_require_money 本次申请退款金额
 * refund_require_num 本次申请数量
 * num 子订单购买数量
 */
function confirmRefund(order_id, order_goods_id, refund_records_id, refund_require_money, refund_require_num, refund_total_money, refund_total_num, num){
	if(countdown_btn != null){
		clearInterval(countdown_btn);
	...
	$("#confirm_refund_records_id").val(refund_records_id);
	$("#refund_require_money").val(refund_require_money);
	$("#apply_num").text(refund_require_num + "件");
	$("#apply_money").text(parseFloat(refund_require_money).toFixed(2) + "元");
	$("#refund_total_num").text(refund_total_num + "件");
	$("#refund_total_money").text(parseFloat(refund_total_money).toFixed(2) + "元");
	$("#buy_num").text(num + "件");
	$("#refund_money_input").next().css("display","none");
	orderGoodsRefundMoney(order_goods_id);//查询买家实际支付金额
	getOrderGoodsRefundBalance(order_goods_id);//查询订单项实际可退款的余额
	getOrderTermsOfPayment(order_id);//查询当前订单的付款方式
