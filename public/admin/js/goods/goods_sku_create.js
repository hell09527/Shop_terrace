...
function createTable(){
	...
	th_html += '<th class="th-code">物料编码</th>';
	...
	for(var i = 0; i < $sku_array.length; i ++){
		...
		if($temp_Obj[child_id_string] == undefined){
			...
			$temp_Obj[child_id_string]["material_code"] ="";
		}
		...
		html +='<td><input type="text" name="material_code" class="js-code input-small" value="'+$temp_Obj[child_id_string]["material_code"]+'"/></td>';
...
function synchroSkuValueData(){
	...
	for(var i = 0; i < $sku_array.length ; i++ ){
		...
		if(sku_str == ""){
			sku_str = sku_id +"¦"+value_array["sku_price"]+"¦"+value_array["market_price"]+"¦"+value_array["cost_price"]+"¦"+value_array["stock_num"]+"¦"+value_array["code"]+"¦"+value_array["material_code"];
		}else{
			sku_str +="§"+sku_id +"¦"+value_array["sku_price"]+"¦"+value_array["market_price"]+"¦"+value_array["cost_price"]+"¦"+value_array["stock_num"]+"¦"+value_array["code"]+"¦"+value_array["material_code"];
		}
	}
...
function updateTempObjData(sku_data){
	...
	if($specObj.length > 0){
		$.each(sku_data,function(c,v){
			...
			$temp_Obj[v.attr_value_items]["material_code"] =v.material_code;
		});
