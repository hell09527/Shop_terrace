...
//					});
	...
	$("input[name='sale_type']").change(function() {
		if ($("input[name='sale_type']:checked").val() == 2) {
			$("#sale-end-time").show();
			$("#delivery-end-time").show();
		} else {
			$("#sale-end-time").hide();
			$("#delivery-end-time").hide();
		}
	});
	//***********************************选择销售方式end***********************************
	...
	//***********************************内购设置***********************************
	//$("input[name='insideSellSelect']").change(function() {
	//	if ($("input[name='insideSellSelect']:checked").val() == 1) {
	//		$("#inside-sell-exchange").show();
	//	} else {
	//		$("#inside-sell-exchange").hide();
	//	}
	//});
	//***********************************内购设置***********************************
	...
	$('input[name="sku_price"],input[name="market_price"],input[name="cost_price"],input[name="stock_num"],input[name="code"]').live('keyup',function() {
		...
		if($this.attr("name") == "sku_price" || $this.attr("name") == "market_price" || $this.attr("name") == "cost_price" || $this.attr("name") == "stock_num"){
			if($this.val().length>0 && reg.test($this.val())){
				if ($this.val().replace(/(^\s*)|(\s*$)/g, "") == "" || $this.val().replace(/(^\s*)|(\s*$)/g, "") < "0.01") {
					$this.css("border-color", "#b94a48");
					$this.parent().find(".help-inline").show();
				} else {
					num = parseInt($this.val());
...
function ValidateUserInput() {
	...
	// 预售设置
	if ($("input[name='sale_type']:checked").val() == 2) {
		var sale_end_time = $("#sale_end_time").val();
		if(!sale_end_time){
			$("#sale_end_time").focus();
			$("#sale_end_time").next("span").show();
			return false;
		}else{
			$("#sale_end_time").next("span").hide();
		}

		var delivery_end_time = $("#delivery_end_time").val();
		if(!delivery_end_time){
			$("#delivery_end_time").focus();
			$("#delivery_end_time").next("span").show();
			return false;
		}else{
			$("#delivery_end_time").next("span").hide();
		}
	}
...
//	
	...
	// }
    //
	//if($("input[name='insideSellSelect']:checked").val() == 1  && $("#txtProductInsidePrice").val() < 0.01){
	//	$("#txtProductInsidePrice").nextAll("span:last").text("请给内购商品设置内购价");
	//	$("#txtProductInsidePrice").nextAll("span:last").show();
	//	$("#txtProductInsidePrice").focus();
	//	return false;
	//}else{
	//	$("#txtProductInsidePrice").nextAll("span:last").hide();
	//}

	if($("#txtProductCodeA").val().length>0  && $("#materialCode").val().length<40){
		$("#txtProductCodeA").next("span").hide();
	}else{
		$("#txtProductCodeA").focus();
		$("#txtProductCodeA").next("span").show();
	...
	if($("#materialCode").val().length>0 && $("#materialCode").val().length<40){
		$("#materialCode").next("span").hide();
	}else{
		$("#materialCode").focus();
		$("#materialCode").next("span").show();
		return false;
	}

	if($("#fraction").val() && $("#fraction").val()>=0 && $("#fraction").val()<=50){
		$("#fraction").next("span").hide();
	}else{
		$("#fraction").focus();
		$("#fraction").next("span").show();
		return false;
	}
			...
			success : function(res) {
				...
				if (res != null) {
					if (type == 1) {
						...
						url = __URL(APPMAIN + "/goods/goodsdetail?id="+parameter_goodsid);// 跳转到前台
						window.open(url);
					}
...
function PackageProductInfo() {
	...
	productViewObj.source_type = $("input[name='source_type']:checked").val();
	productViewObj.goods_type = $("input[name='goods_type']:checked").val();
	productViewObj.sale_type = $("input[name='sale_type']:checked").val();// 销售方式
	...
	productViewObj.vip_price = $("#txtProductVipPrice").val().replace(/^\s*/g, "").replace(/\s*$/g, "") == "" ? 0 : $("#txtProductVipPrice").val().replace(/^\s*/g, "").replace(/\s*$/g,"");// 会员价
	...
	productViewObj.is_vip = $("input[name='is_vip']:checked").val();//会员专区
	productViewObj.is_black_tech = $("input[name='is_black_tech']:checked").val();//黑科技排行榜
	productViewObj.new_pic = $("#logo").val();//新品推荐图片
	productViewObj.material_black = $("#material_black").val();//原料黑科技
	productViewObj.use_black = $("#use_black").val();//使用黑科技
	productViewObj.effect_black = $("#effect_black").val();//效果黑科技
	productViewObj.selling_point = $("#selling_point").val();// 黑科技卖点
	// productViewObj.product_code = $("#productCode").val();// 货品编码
	productViewObj.code = $("#txtProductCodeA").val();// 商品编码
	//productViewObj.product_barcode = $("#productBarcode").val();// 商品条形码
	productViewObj.material_code = $("#materialCode").val();// 物料编码
	productViewObj.fraction = $("#fraction").val();// 分润比率
	...
	//预售
	productViewObj.sale_end_time = $("#sale_end_time").val();
	productViewObj.delivery_end_time = $("#delivery_end_time").val();
