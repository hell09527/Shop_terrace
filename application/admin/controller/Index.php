...
class Index extends BaseController
{
    ...
    public function index()
    {
        ...
        if($this->group_id == 6 || $this->group_id == 7 || $this->group_id == 9|| $this->group_id == 11){
            return view($this->style . 'Index/indexOther');
        }
     ...
     * 2017年7月10日 12:06:01 王永杰
     */
    public function getKolBalanceWithdrawCount()
    {
        $kol_list = \think\Db::name('bc_distributor')->where(['is_check' => 0 ,'origin' => 1])->select();
        $count    = 0;
        if(!empty($kol_list)){
            $count = count($kol_list);
        }
        return $count;
    }
    ...
    public function getSalesStatistics()
    {
        ...
        https://store-test.91xdb.com/index.php/admin/goods/getsearchgoodslist
        # 查看昨天 todo @陈 , 修改昨天查询条件
        $yesterday_condition = $tmp_condition;
        $yesterday_condition['create_time'] = [
            'between',
            [
                strtotime( date('Y-m-d',time()-24*60*60)." 00:00:00" ),
                strtotime( date('Y-m-d',time()-24*60*60)." 23:59:59" )
            ]
        ];
        $data['yesterday_money'] = $order->getPayMoneySum($yesterday_condition);
        $data['yesterday_goods'] = $order->getGoodsNumSum($yesterday_condition);
        // 查看本月
        $tmp_condition = $condition;
