...
//    }
    ...
    public function memberWithdrawSetting()
    {
        ...
        if (request()->isAjax()) {
            $shop_id              = $this->instance_id;
            $key                  = 'WITHDRAW_BALANCE';
            $withdraw_account_arr = request()->post("withdraw_account", "2");
            $withdraw_account_arr = explode(",", $withdraw_account_arr);
            $withdraw_account     = array(
                array(
                    'id' => 'wechat',
            ...
            foreach ($withdraw_account_arr as $v) {
                $withdraw_account[$v - 2]['is_checked'] = 1;
            }
            $value  = array(
                'withdraw_cash_min' => request()->post('cash_min', 0),
                'withdraw_cash_max' => request()->post('cash_max', 0),
                'withdraw_poundage' => request()->post('poundage', 0),
                'withdraw_message' => request()->post('message', ''),
                'withdraw_account' => $withdraw_account
            );
    ...
    public function wapCustomTemplateEdit()
    {
        ...
        // $ds=$config->getDefaultWapCustomTemplate();
        if (empty($custom_template_info) && $id) {
            ...
            $this->assign('ajax_add_custom_template', __URL(addons_url('customtemplate://customtemplate/addCustomTemplate')));
            $this->assign('ajax_updateCustomTemplate', __URL(addons_url('customtemplate://customtemplate/updateCustomTemplate')));

            $this->assign('custom_template_list_url', __URL(ADMIN_MAIN . '/Menu/addonmenu?addons=customtemplate&parameter=list'));
