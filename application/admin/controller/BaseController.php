...
class BaseController extends Controller
{
    ...
    public function init()
    {
        ...
        if ($check_auth) {
            ...
            if (empty($web_info['web_popup_title'])) {
                $this->assign("web_popup_title", "Bonnie&Clyde");
            } else {
     ...
     * 管理员操作日志
     */
    public function admin_user_record($title,$id,$res){
        if(is_array($id)){
            $id = '';
        }
        if(strstr($title,'查看') == false){
            $this->insetLog($title,$id);
        }else{
            $data       = \think\Db::name('bc_admin_user_log')->where(['status' => 1,'name' => $title])->order('create_time desc')->select();
            $time_limit = $data[0]['create_time'] + 18000;
            if($time_limit < time()){
                $this->insetLog($title,$id);
            }
        }
    ...
    public function insetLog($title,$id){
        $data['name']        = $title;
        $data['uid']         = $_SESSION['niu']['adminuid'];
        $data['model_url']   = $this->module_info['url'];
        $data['is_success']  = 1;
        $data['operate_id']  = $id;
        $data['operate_ip']  = $this->get_ip();
        $data['create_time'] = time();
        \think\Db::name('bc_admin_user_log')->insert($data);
    }
