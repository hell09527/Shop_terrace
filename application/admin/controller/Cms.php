...
//    }
    ...
    public function topicList()
    {
        if (request()->isAjax()) {
            ...
            $page_size   = request()->post('page_size', PAGESIZE);
            $search_text = request()->post("search_text", "");
            $retval      = $article->getAdminTopicList($page_index, $page_size, [
                'title' => array(
                    "like",
                    "%" . $search_text . "%"
                )
...
//    }
    ...
    public function addTopic()
    {
        if (request()->isAjax()) {
            ...
            $res              = $data['items'];
            $topic            = $article->addMasterTopic($instance_id, $ret);
            if ($res) {
                foreach ($res as $v) {
                    unset($v['id']);
                    $v['master_id'] = $topic;
                    $article->addSlaverTopic($instance_id, $v);
                }
            }
            return AjaxReturn($topic);
        } else {
...
//            $info = $article->getMasterTopicDetail($topic_id);
//            $info['items'] = $article->getSlaverTopicDetail($info['id']);
...
//    }
    ...
    public function updateTopic()
    {
        ...
        if (request()->isAjax()) {
            ...
            $ret['status']    = $data['status'];
            $res              = $data['items'];
            $topic            = $article->updateMasterTopic($instance_id, $ret);
            $ids              = $article->getSlaverTopicIds($ret['id']);
            if ($res) {
            ...
            if ($res) {
                foreach ($res as $v) {
                    if ($v['sid']) {
                        ...
                        $article->updateSlaverTopic($instance_id, $v);
                    }
                }
            }
            return AjaxReturn($topic);
    ...
    {
        $id   = request()->post('master_id', 0);
        $info = \think\Db::name('nc_cms_master_topic')->where(['id' => $id])->find();
        #dev
        #$url = "https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid=".'wxba4f30503c1ea656'."&secret=".'369964cf3f3da0a50cd7a9d27ca5577a';
        #prod
        ...
        $post_data      =
            array(
                'scene' => $info['id'],
                'page'  => 'pages/index/projectIndex/projectIndex',
                'width' => 430,
            );
    ...
    public function brandStoryList()
    {
