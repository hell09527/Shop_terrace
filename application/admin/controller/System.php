...
class System extends BaseController
{
    ...
    public function addVideo()
    {
        ...
        $this->assign('child_menu_list', $child_menu_list);
        $videoServe = new Video();
        if (request()->isAjax()) {
            $video_id  = request()->post("video_id", 0);
            $video_name  = request()->post("video_name", "");
            $video_address   = request()->post("video_address", "");
            $res        = $videoServe->setVideo($video_id, $video_name, $video_address);
            return AjaxReturn($res);
        }else{

            $videoDetail = $videoServe->getVideoDetail();
        ...
        return view($this->style . "System/addVideo");
    }
