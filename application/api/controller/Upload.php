...
class Upload extends BaseController
{
    ...
    public function uploadFile()
    {
        ...
        if ($ok["code"]) {
            ...
            // 文件上传成功执行下边的操作
            if (!strstr($this->reset_file_path, UPLOAD_VIDEO) && !strstr($this->reset_file_path, GOODS_VIDEO_PATH)) {
    ...
    private function ajaxFileReturn()
    {
        ...
        return $this->outMessage($title, $this->return);
    }
    ...
    private function validationFile()
    {
        ...
        switch ($this->file_path) {
            case UPLOAD_GOODS:
                ...
                // 商品图片
                if (($this->file_type != "image/gif" && $this->file_type != "image/png" && $this->file_type != "image/jpeg" && $this->file_type != "image/jpeg") || $this->file_size > 1024 * 1024 * 3) {
            ...
            case UPLOAD_GOODS_SKU:
                ...
                // 商品SKU图片
                if (($this->file_type != "image/gif" && $this->file_type != "image/png" && $this->file_type != "image/jpeg" && $this->file_type != "image/jpeg") || $this->file_size > 1024 * 1024) {
            ...
            case UPLOAD_AVATOR:
                ...
                // 用户头像
                if (($this->file_type != "image/gif" && $this->file_type != "image/png" && $this->file_type != "image/jpeg" && $this->file_type != "image/jpg") || $this->file_size > 1024 * 1024) {
                    $size                    = $this->file_size / 1024 / 1024 > 1 ? $this->file_size / 1024 / 1024 : $this->file_size / 1024;
                    $size                    = number_format($size, 2);
                    $unit                    = $this->file_size / 1024 / 1024 > 1 ? 'M' : 'KB';
                    $this->return['message'] = '文件上传失败,请检查您上传的文件类型,文件大小不能超过1MB。您上传的文件类型：' . $this->file_type . ',文件大小：' . $size . $unit;
                    $flag                    = false;
                }
                break;
            case UPLOAD_PAY:
            ...
            case UPLOAD_VIDEO:
                ...
                // 公共视频
                if ($this->file_type != "video/mp4" || $this->file_size > 1024 * 1024 * 50) {
            ...
            case UPLOAD_COMMENT:
                // 评论
                if (($this->file_type != "image/gif" && $this->file_type != "image/png" && $this->file_type != "image/jpg" && $this->file_type != "image/jpeg") || $this->file_size > 1024 * 1024 * 8) {
                    $this->return['message'] = '文件上传失败,请检查您上传的文件类型,文件大小不能超过8MB';
                    $flag                    = false;
                }
                break;
            case UPLOAD_WXCODE:
     ...
     * 用于相册多图上传
     *
     * @return string|multitype:string boolean, number, \think\false, string>
     */
