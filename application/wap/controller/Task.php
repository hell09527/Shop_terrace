...
class Task extends Controller
{
    ...
    public function autoTask()
    {
        ...
        $retval_giftmansong_operation = $event->giftMansongClose();
        $retval_discount_operation = $event->discountOperation();
        $retval_auto_coupon_close = $event->autoCouponClose();
        $retval_auto_coupon_day_close = $event->autoCouponDayClose();
        //内购数量扣减 author:dai
        $neigou_sku_order_separation = $event->promotionGoodsLimit();
        ...
        //结算订单分润 author:Fuxt
        $retval_settlement_order_separation = $event->settlementOrderSeparation();
...
//          $retval_giftOrder_refundAskfor = $event->giftOrderGoodsRefundAskfor();
            $retval_employee_data = $event->updateEmployeeData();
    ...
    {
        $time = time() - 86400;
        $cache = Cache::get("niushop_days_task");
        if (! empty($cache) && $time < $cache) {
            return 1;
        } else {
            $event = new Events();
            $retval_send_mail = $event->sendMail();//发邮件
            Cache::set("niushop_days_task", time());
            return 1;
        }
    }
