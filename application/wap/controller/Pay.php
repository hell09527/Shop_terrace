...
class Pay extends Controller
{
    ...
    public function wchatUrlBack()
    {
        ...
        if (! empty($postStr)) {
            ...
            if ($postObj->result_code == 'SUCCESS' && $check_sign == 1) {
                ...
                $retval = $pay->onlinePay($postObj->out_trade_no, $postObj->transaction_id, 1, '');
                ...
                if($orderInfo['order_status'] == 1){
                    $order = new Order();
                    $order->pushOrderIdByOutTradeNo($postObj->out_trade_no); //付款推送order_id给OMS
                    $order->orderDataImport($postObj->out_trade_no); //订单推送给小程序
                }
                $xml = "<xml>
    ...
    {
        $unify = new UnifyPay();
        $result = $unify->aliayTransfers("2018110160425", "18735107960", 0.1);
        var_dump($result);
    }
}
