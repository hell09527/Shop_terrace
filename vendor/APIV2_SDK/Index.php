...
//机器人题库
//问题列表
//$data = $test->kchat()->findAll();
//创建问题
//	$info = array('category_id'=>119,'title'=>'test','answer'=>'收到','active'=>1);
//	$data = $test->kchat()->create($info);
//工单（客服）
/* 	 $ids = array('1530','1531');
 */
/* 	$json = array(
...
//	触发器列表
//	$data = $test->triggers()->findAll();
//	获取指定触发器
//	$data = $test->triggers()->find('341652');
// 	获取已启用的触发器列表
//	$data = $test->triggers()->active();


//自动化任务

//	自动化任务列表
//	$data = $test->automations()->findAll();
//	获取指定触发器
//	$data = $test->automations()->find('999');
// 	获取已启用的触发器列表
//	$data = $test->automations()->active();	
