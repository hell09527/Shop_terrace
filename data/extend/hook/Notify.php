...
class Notify
{
    ...
    public function orderDelivery($params = null)
    {
        ...
        $_order_id = \think\Db::name('ns_order_goods')->where(['order_goods_id' => $order_goods_str[0]])->find()['order_id'];
        #todo  。。。 判断订单来源 判断是否发送短信
        $_order_info = \think\Db::name('ns_order')->where(['order_id' => $_order_id])->find();

        $buyerIdArray = [5546,5553,5554,5555];
        if (count($order_goods_str) > 0 && $_order_info['source_distribution'] != 4560 && in_array($_order_info['buyer_id'],$buyerIdArray) ) {
            ...
            /**
             * 模版CODE: SMS_137420905
             * ${username}在${shopname}的订单${orderno}已经通过${expresscompany}发货,
             * 快递号为${expressno}.
             * 添加BC店长号：BonnieClydeHunt, 享一对一VIP服务.
             */
            $sms_params          = array(
//                "shop_name"       => $this->shop_name,
//                "user_name"       => $user_name,
...
//                "express_no"      => $express_obj["express_no"],
                "shopname"        => 'Bonnie&Clyde',
                "username"        => $_order_info['receiver_name'],
                "orderno"         => $order_no,
...
//            }
            ...
            #极光 短信发送
            if (!empty($mobile) && $this->mobile_is_open == 1) {
                $app_key       = 'e9b6719b7efbb89fedde9301';
                $master_secret = 'efb08f33e3122e3aa4cf796c';
                $temp_id       = '163959';
                $sign_id       = '8782';
                $temp_para     = [
                    'username'       => $_order_info['receiver_name'],
                ...
                require (VENDOR_PATH.'jsms/src/JSMS.php');
                $client = new \JiGuang\JSMS($app_key, $master_secret);
                $client->sendMessage($mobile, $temp_id, $temp_para, $time = null, $sign_id);
            }
     ...
     * @param string $params
     */
    public function checkKf($params = null)
    {
        $rand    = rand(1000, 9999);
        $mobile  = $params["mobile"];
        $shop_id = $params["shop_id"];
