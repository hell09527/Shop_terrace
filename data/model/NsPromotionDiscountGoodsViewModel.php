...
class NsPromotionDiscountGoodsViewModel extends BaseModel {
    ...
    {
        //设置查询视图
        # todo @陈 添加查询字段 ns_goods.picture
        $viewObj = $this->alias('npdg')
        ->join('ns_goods ng','ng.goods_id = npdg.goods_id','inner')
        ->field('npdg.discount_goods_id,npdg.discount_id,npdg.start_time,npdg.end_time,npdg.goods_id,npdg.status,npdg.discount,npdg.goods_name,npdg.goods_picture,ng.category_id_1,ng.picture,ng.price,ng.promotion_price,ng.market_price')
        ->group('ng.goods_id');
    ...
    public function getGoodsViewQueryField($condition, $field, $order=""){
        $viewObj = $this->alias('npdg')
            ->join('ns_goods_sku ngs','npdg.sku_id = ngs.sku_id','left')
            ->field($field);
