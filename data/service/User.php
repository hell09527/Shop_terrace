...
class User extends BaseService implements IUser
{
     ...
     * 获取会员信息通过unionid
     */
    public function getUserDetailByUnionid($unionid)
    {
        $user_info = $this->user->getInfo(['wx_unionid'=>$unionid],'uid,instance_id');
        return $user_info;
    }
    /**
     * 
...
/**
    ...
    public function addSourceBranch($uid, $store_id)
    {
        $condition = array(
            'uid' => $uid
        );
        $user_info = $this->user->getInfo($condition, $field = 'uid, user_status, is_member, source_branch, source_branch_time');
        if (! empty($user_info)) {
            if ($user_info['user_status'] == 0) {
                return USER_LOCK;
            } else {
                if($user_info['source_branch'] == 0 && $store_id > 0){
                    $source_branch = $store_id;
                    $source_branch_time = time();
                }else if($user_info['source_branch'] > 0){
                    $source_branch = $user_info['source_branch'];
                    $source_branch_time = $user_info['source_branch_time'];
                }else{
                    $source_branch = $store_id;
                    $source_branch_time = 0;
                }
                $data = array(
                    'source_branch' => $source_branch,
                    'source_branch_time' => $source_branch_time
                );
                $this->user->save($data,['uid' => $user_info['uid']]);
                return 1;
            }
        } else
            return USER_NBUND;
    ...
    public function updateUserRealName($uid, $name)
    {
        $retval = $this->user->save(['real_name' => $name], ['uid' => $uid]);
        return $retval;
...
//    public function getUserInfoDetail($uid){
//        $member = new NsMemberModel();
//        return $user_info = $member->getInfo(array("uid"=>$uid));
//    }
