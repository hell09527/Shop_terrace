...
class Goods extends BaseService implements IGoods
{
     ...
     */
    public function getGoodsList($page_index = 1, $page_size = 0, $condition = '', $order = 'ng.sort asc')
    {
        #todo @dai 新增缓存
//        $data = array($page_index = 1, $page_size = 0, $condition = '', $order = 'ng.sort asc,ng.create_time desc');
//        $data = json_encode($data);
//
//        $cache = Cache::tag("niu_goods_pro")->get("getGoodsBrandProList".$data);
//        if(empty($cache))
//        {
...
//            }
        ...
        if (!empty($list['data'])) {
            ...
            foreach ($list['data'] as $k => $v) {
                ...
                $list["data"][$k]['price']          = $list["data"][$k]['promotion_price'] = $list["data"][$k]['sku_list'][0]['promote_price'];
...
//            Cache::tag("niu_goods_pro")->set("getGoodsBrandProList".$data, $list);
        return $list;
//        }else{
//            return $cache;
//        }
    ...
    public function getGoodsDataList($condition = '', $order = 'ng.sort asc')
    {
        $goods_model = new NsGoodsModel();
        // 针对商品分类
        $data           = $goods_model->alias("ng")
            ->join('ns_goods_brand ngb', 'ngb.brand_id = ng.brand_id', 'left')
            ->where($condition)
            ->order($order)
            ->field("ng.goods_id,ng.goods_name,ng.source_type,ng.state,ng.category_id_1,ng.category_id_2,ng.category_id_3,ng.fraction,ng.goods_type,ngb.brand_name,ng.code,ng.material_code,ng.market_price,ng.price,ng.stock,ng.min_stock_alarm,ng.sale_type,ng.wx_code_online,ng.wx_code_store")
            ->select();
        $source_type    = ['1' => '大贸', '2' => '跨境'];
        $sale_type      = ['1' => '现售', '2' => '预售'];
        $state          = ['0' => '下架', '1' => '上架', '10' => '禁售'];
        $goods_type     = ['0' => '虚拟商品', '1' => '实物商品', '2' => '实物礼品', '3' => '会员'];
        $goods_category = new NsGoodsCategoryModel();
        foreach ($data as $k => $v) {
            $data[$k]['source_type_name'] = $source_type[$v['source_type']];
            $data[$k]['sale_type_name']   = $sale_type[$v['sale_type']];
            $data[$k]['state_name']       = $state[$v['state']];
            $data[$k]['fraction_name']    = ($v['fraction'] * 100) . '%';
            $data[$k]['goods_type_name']  = $goods_type[$v['goods_type']];
            $data[$k]['category_name']    = '';
            if ($v['category_id_1']) {
                $info = $goods_category->get($v['category_id_1']);
                $data[$k]['category_name'] .= $info['category_name'];
            }
            if ($v['category_id_2']) {
                $info = $goods_category->get($v['category_id_2']);
                $data[$k]['category_name'] .= '>' . $info['category_name'];
            }
            if ($v['category_id_3']) {
                $info = $goods_category->get($v['category_id_3']);
                $data[$k]['category_name'] .= '>' . $info['category_name'];
            }
        }
        return $data;
    ...
    public function goodsExcelList($condition)
    {
        $goods_sku_model = new NsGoodsSkuModel();
        // 针对商品分类
        $data = $goods_sku_model->alias("ngs")
            ->join('ns_goods ng', 'ng.goods_id = ngs.goods_id', 'left')
            ->join('ns_goods_brand ngb', 'ngb.brand_id = ng.brand_id', 'left')
            ->where($condition)
            ->order('ng.goods_id')
            ->group('ngs.material_code')
            ->field("ngs.code,ngs.material_code,ngs.sku_name,ngs.stock,ng.goods_id,ng.goods_name,ng.source_type,ng.state,ng.goods_type,ng.wx_code_online,ng.wx_code_store,ng.category_id_1,ng.category_id_2,ng.category_id_3,ngb.brand_name")
            ->select();
        $source_type    = ['1' => '大贸', '2' => '跨境'];
        $state          = ['0' => '下架', '1' => '上架', '10' => '禁售'];
        $goods_type     = ['0' => '虚拟商品', '1' => '实物商品', '2' => '实物礼品', '3' => '会员'];
     ...
     * 获取限时折扣商品
     */
    public function getDiscountList($page_index = 1, $page_size = 0, $condition = '', $order = '', $field = '*', $material_code)
    {
        $goods_id_array = [];
        if($material_code!=''){
            $goods_sku = (new NsGoodsSkuModel())
                ->where(['material_code'=>['like',"%$material_code%"]])
                ->field('sku_id, goods_id')
                ->select();
            $goods_id_array = array_column($goods_sku,'goods_id');
        }
        if(!empty($goods_id_array)){
            $condition = array_merge($condition,['goods_id'=>['in',$goods_id_array]]);
        }
        $result = $this->goods->pageQuery($page_index, $page_size, $condition, $order, $field);
        $goods_type = ['0' => '虚拟商品', '1' => '实物商品', '2' => '实物礼品', '3' => '会员'];
        $goods_state = ['0' => '已下架', '1' => '已上架'];
        foreach ($result['data'] as $k => $v) {
            $result['data'][$k]['goods_type_name'] = $goods_type[$v['goods_type']] ;//商品类型
            $result['data'][$k]['goods_state_name'] = $goods_state[$v['state']] ;//上下架
            ...
            //商品分类
            $categoryInfo = \think\Db::name('ns_goods_category')->where(['category_id' => $v['category_id_1']])->find();
            if(!empty($categoryInfo)){
                $result['data'][$k]['category_name'] = $categoryInfo['category_name'];
            }else{
                $result['data'][$k]['category_name'] = '';
            }

            //商品sku
            $sku = new NsGoodsSkuModel();
            $result['data'][$k]['sku_list'] = $sku->getQuery(['goods_id'=>$v['goods_id']], 'sku_id,sku_name,picture,material_code,price,stock', '');
        }
        return $result;
    }
    ...
    function goods_wxcode_make($goodsId, $url_online, $url_store)
    {
        $data   = array(
            'wx_code_online' => $url_online,
            'wx_code_store' => $url_store
        );
        $result = $this->goods->save($data, [
            'goods_id' => $goodsId
        ]);
    ...
    function goods_code_make($goodsId, $url, $url_online, $url_store)
    {
        $data   = array(
            'QRcode' => $url,
            'wx_code_online'=>$url_online,
     ...
     */
    public function addOrEditGoods($goods_id, $goods_name, $shopid, $category_id, $category_id_1, $category_id_2, $category_id_3, $supplier_id, $brand_id, $group_id_array, $source_type, $goods_type, $market_price, $price, $vip_price, $point_exchange_type, $point_exchange, $give_point, $is_member_discount, $shipping_fee, $shipping_fee_id, $stock, $max_buy, $min_buy, $min_stock_alarm, $sales, $collects, $star, $evaluates, $province_id, $city_id, $picture, $keywords, $introduction, $description, $QRcode, $is_vip, $is_black_tech, $material_black, $use_black, $effect_black, $selling_point, $code, $material_code, $fraction, $is_stock_visible, $is_hot, $is_recommend, $is_new, $sort, $image_array, $sku_array, $state, $sku_img_array, $goods_attribute_id, $goods_attribute, $goods_spec_format, $goods_weight, $goods_volume, $shipping_fee_type, $extend_category_id, $sku_picture_values, $virtual_goods_type_id, $production_date, $shelf_life, $ladder_preference, $goods_video_address, $new_pic, $sale_type, $sale_end_time, $delivery_end_time)
    {
        Cache::tag("niu_goods_group")->clear();
        Cache::tag("niu_goods_category_block")->clear();
        Cache::tag("niu_goods")->clear();
//        Cache::tag("niu_goods_pro")->clear();
        $error         = 0;
        ...
        //预售处理
        if ($sale_type == 2) {
            $sale_end_time = getTimeTurnTimeStamp($sale_end_time);
            $delivery_end_time = getTimeTurnTimeStamp($delivery_end_time);
        }else{
            $sale_end_time = 0;
            $delivery_end_time = 0;
        }
...
//        Cache::tag("niu_goods_pro")->clear();
    ...
    public function getBasisGoodsDetail($goods_id)
    {
        ...
        #todo  @dai   内购价格   goods_id -> sku_id   内购价格放入sku_list
        $promotion_info = \think\Db::name('ns_promotion_mansong')->where(['is_neigou' => 1, 'status' => 1])->find();
        $member         = new Member();
        $member_info    = $member->getMemberIsEmployee();       #判断是否内部员工
        ...
        // 查询商品sku      todo @dai sku新增内购价格
        $goods_sku        = new NsGoodsSkuModel();
        $NG_MODEL         = new NsPromotionNeigouGoodsModel();
        $goods_sku_detail = $goods_sku->where('goods_id=' . $goods_id)->select();
        foreach($goods_sku_detail as $key=>$value){
            $goods_sku_detail[$key]['is_inside_sell'] = 0;
            $price = $goods_sku_detail[$key]['price'];
            $goods_sku_detail[$key]['price']          = $goods_sku_detail[$key]['promote_price'];
            if ($member_info == 1) {
                if ($promotion_info) {
                    #满足内购
                    $condition['discount_id'] = $promotion_info['mansong_id'];
                    $condition['goods_id']    = $goods_detail['goods_id'];
                    $condition['sku_id']      = $goods_sku_detail[0]['sku_id'];
                    #获取默认价格
                    $default_promotion              = $NG_MODEL->getInfo($condition);
                    if(!empty($default_promotion)){
                        $sku_detail = $goods_sku->where(['sku_id'=>$condition['sku_id']])->find();
                        $n_price = $default_promotion['n_price'] == 0 ?
                            $default_promotion['n_discount'] * $sku_detail['price'] / 10 :
                            $default_promotion['n_price'];
                        $goods_detail['inside_price']   =  sprintf("%.2f", $n_price);;
                        $goods_detail['is_inside_sell'] = '1';
                    }else{
                        $goods_detail['inside_price']   = '';
                        $goods_detail['is_inside_sell'] = '0';
                    }
                    $condition1['discount_id']      = $promotion_info['mansong_id'];
                    $condition1['goods_id']         = $goods_detail['goods_id'];
                    $condition1['sku_id']           = $value['sku_id'];
                    $promotion_goods_info           = $NG_MODEL->getInfo($condition1);
                    if ($promotion_goods_info) {
                        $goods_sku_detail[$key]['is_inside_sell'] = 1;
                        $n_price = $promotion_goods_info['n_price'] == 0 ?
                            $promotion_goods_info['n_discount'] * $price / 10 :
                            $promotion_goods_info['n_price'];
                        $goods_sku_detail[$key]['inside_price']   = sprintf("%.2f", $n_price);
//                        $goods_sku_detail[$key]['stock']          = $promotion_goods_info['sku_num'] - $promotion_goods_info['use_num'];
                    }
                }
            }
        }
        ...
        $goods_detail['price'] = $goods_detail['promotion_price'] = $goods_sku_detail[0]['promote_price'];  # 设置默认价格   #戴
            ...
            foreach ($spec_list as $k => $v) {
                ...
                foreach ($v["value"] as $m => $t) {
                    ...
                    if (!empty($sku_img_array)) {
                        ...
                        $picture = $array[0];
                        $spec_value_src = $sku_img_array['album_picture_list'][0];
                    }
                    // 查询SKU规格主图，没有返回0
                    $spec_list[$k]["value"][$m]["picture"] = $picture;
                    $spec_list[$k]["value"][$m]["spec_value_src"] = $spec_value_src;
                    // 规格图片
                    // 判断规格数组中图片路径是id还是路径
	...
	$goods_detail['category_show'] = 0;
	$category_info_1 = \think\Db::name('ns_goods_category')->where(['category_id' => $goods_detail['category_id_1']])->find();
	if($category_info_1['is_visible'] == 1){
		 $goods_detail['category_show'] = 1; 
	}
    ...
    // 首页商品详情图片列表
    public function goodsDetailImgList()
    {
        $imgList = [
            [
                'goods_id' => 92,
                'imgDetailUrl' => 'https://static.bonnieclyde.cn/news92.jpg',
            ],
            [
                'goods_id' => 123,
                'imgDetailUrl' => 'https://static.bonnieclyde.cn/news123.jpg',
            ],
            [
                'goods_id' => 126,
                'imgDetailUrl' => 'https://static.bonnieclyde.cn/news126.jpg',
            ],
            [
                'goods_id' => 200,
                'imgDetailUrl' => 'https://static.bonnieclyde.cn/news200.jpg',
            ],
            [
                'goods_id' => 276,
                'imgDetailUrl' => 'https://static.bonnieclyde.cn/news276.jpg',
            ],
            [
                'goods_id' => 277,
                'imgDetailUrl' => 'https://static.bonnieclyde.cn/news277.jpg',
            ],
            [
                'goods_id' => 282,
                'imgDetailUrl' => 'https://static.bonnieclyde.cn/news282.jpg',
            ]
        ];
        return $imgList;
    }
...
//        $goods_detail['mansong_name'] = $goods_mansong->getGoodsMansongName($goods_detail["goods_id"]);
        $sku_info = \think\Db::name('ns_goods_sku')->where(['goods_id' => $goods_detail["goods_id"]])->find();
        $goods_detail['mansong_name'] = $goods_mansong->getGoodsMansongName($sku_info['sku_id']);
        $goods_detail['mansong_name_array'] = $goods_mansong->getGoodsMansongNameArray($sku_info['sku_id']);
        // 查询包邮活动
        $full        = new Promotion();
        ...
        if (!empty($this->uid)) {
            ...
            //     )
            // ], "num");
            $where                        = ["nog.goods_id" => $goods_detail["goods_id"], "nog.buyer_id" => $this->uid, "no.order_status" => array("neq", 5)];
            $num                          = $orderGoods->getorderGoodsNum($where);
...
//        $goods_detail['mansong_name'] = $goods_mansong->getGoodsMansongName($goods_id);
        $sku_info = \think\Db::name('ns_goods_sku')->where(['goods_id' => $goods_id])->find();
        $goods_detail['mansong_name'] = $goods_mansong->getGoodsMansongName($sku_info['sku_id']);
        // 查询包邮活动
        $full        = new Promotion();
        $baoyou_info = $full->getPromotionFullMail($this->instance_id);
     ...
     */
    public function getSearchGoodsList($page_index = 1, $page_size = 0, $condition = '', $order = '', $field = '*', $search_goods_sku = '')
    {
        $goods_id_array = [];
        $sku_id_array   = [];
        if($search_goods_sku!=''){
            $goods_sku = (new NsGoodsSkuModel())
                ->where(['material_code'=>['like',"%$search_goods_sku%"]])
                ->field('sku_id, goods_id')
                ->select();
            $goods_id_array = array_column($goods_sku,'goods_id');
            $sku_id_array   = array_column($goods_sku,'sku_id');
        }
        if(!empty($goods_id_array)){
            $condition = array_merge($condition,['goods_id'=>['in',$goods_id_array]]);
        }
        ...
        foreach ($result['data'] as $k => $v) {
            ...
            $result['data'][$k]['picture_info']     = $pic_info;
            $result['data'][$k]['category_id_name'] = \think\Db::name('ns_goods_category')->where(['category_id' => $v['category_id_1']])->find()['category_name'];
            if ($v['goods_type'] == 0) {
                $result['data'][$k]['goods_type'] = '虚拟商品';
            } else if ($v['goods_type'] == 1) {
                $result['data'][$k]['goods_type'] = '实物商品';
            } else if ($v['goods_type'] == 2) {
                $result['data'][$k]['goods_type'] = '实物礼品';
            } else {
                $result['data'][$k]['goods_type'] = '会员';
            }
            ...
            if(!empty($sku_id_array)){
                $sku_condition = array_merge([
                    'sku_id'=>['in',$sku_id_array]
                ], $sku_condition);
            }
            $result['data'][$k]['sku_arr']        = $sku->getQuery($sku_condition, 'sku_id,material_code,sku_name,price,stock,picture', '');
            $result['data'][$k]['select_sku_arr'] = [];
        }
        return $result;
    }
     ...
     * (non-PHPdoc)
     *
     * @see \data\api\IGoods::getPromotionGoodsList()
     */
    public function getPromotionGoodsList($page_index = 1, $page_size = 0, $condition = '', $order = '', $field = '*')
    {
        $result = $this->goods->pageQuery($page_index, $page_size, $condition, $order, $field);
        $goods_type = ['0' => '虚拟商品', '1' => '实物商品', '2' => '实物礼品', '3' => '会员'];
        $goods_state = ['0' => '已下架', '1' => '已上架'];
...
//        Cache::tag('niu_goods_pro')->clear();
     ...
     * 分润设置
     */
    public function modifygoodsFraction($goods_ids, $fraction)
    {
        $goods = new NsGoodsModel();
        $goods->startTrans();
        try {
            ...
            foreach ($goods_id_array as $k => $v) {
                $goods         = new NsGoodsModel();
//                $goods_info    = $goods->getInfo(['goods_id' => $v], 'category_id');
//                $category      = new NsGoodsCategoryModel();
//                $category_info = $category->getInfo(['category_id' => $goods_info['category_id']], 'is_visible');
//                if ($category_info['is_visible'] == 0 && $goods_info['category_id'] != 39) {
//                    $data = array("fraction" => 0);
//                } else {
//                    $data = array("fraction" => $fraction / 100);
//                }
                $data = array("fraction" => $fraction / 100);
                $goods->save($data, [
                    'goods_id' => $v
     ...
     * @param unknown $boxes
     */
    public function getBoxList($boxes)
    {
        $box       = new NsBoxModel();
        $box_list  = $box->getQuery([
            'buyer_id' => $this->uid
        ], '*', 'box_id');
        $box_array = explode(',', $boxes);
        $list      = array();
        foreach ($box_list as $k => $v) {
            ...
            if ($num != $v['num']) {
                // 更新礼品盒
                $this->boxAdjustNum($v['box_id'], $sku_info['stock']);
                $v['num'] = $num;
            }
            ...
            if (in_array($v['box_id'], $box_array)) {
                $list[] = $v;
            }
        }
        return $list;
    ...
    public function getCartList($carts)
    {
        ...
        foreach ($cart_list as $k => $v) {
            ...
            $goods_info = $goods->getInfo([
                'goods_id' => $v['goods_id']
            ], 'max_buy,state,point_exchange_type,point_exchange,sale_type,sale_end_time,delivery_end_time');
            // 获取商品sku信息
            $goods_sku = new NsGoodsSkuModel();
            $sku_info  = $goods_sku->getInfo([
                'sku_id' => $v['sku_id']
            ...
            $v['point_exchange_type'] = $goods_info['point_exchange_type'];
            $v['point_exchange']      = $goods_info['point_exchange'];
            $v['sale_type']      = $goods_info['sale_type'];
            $v['sale_end_time']      = $goods_info['sale_end_time'];
            $v['delivery_end_time']      = $goods_info['delivery_end_time'];
            if ($goods_info['state'] != 1) {
     ...
     * @param unknown $uid
     */
    public function getBox($uid, $shop_id = 0)
    {
        if ($uid > 0) {
            $box            = new NsBoxModel();
            $box_goods_list = null;
            if ($shop_id == 0) {
                $box_goods_list = $box->getQuery([
                    'buyer_id' => $this->uid
                ], '*', '');
            } else {
            ...
            $box_goods_list = cookie('box_array');
            if (empty($box_goods_list)) {
                $box_goods_list = array();
            } else {
                $box_goods_list = json_decode($box_goods_list, true);
            }
        }
        $goods_id_array = array();
        if (!empty($box_goods_list)) {
            foreach ($box_goods_list as $k => $v) {
                $goods      = new NsGoodsModel();
                $goods_info = $goods->getInfo([
                    'goods_id' => $v['goods_id']
                ], 'max_buy,state,point_exchange_type,point_exchange,goods_name,price, picture, min_buy, source_type');
                // 获取商品sku信息
                $goods_sku = new NsGoodsSkuModel();
                $sku_info  = $goods_sku->getInfo([
                    'sku_id' => $v['sku_id']
                ], 'stock, price, vip_price, sku_name, promote_price');
...
//                    }
                    if (!empty($this->uid)) {
                        $is_vip = $goods_preference->getMemberIsVip($uid);
                    } else {
                        $is_vip = 0;
                    }
                    if ($is_vip == 1 && $sku_info['vip_price'] > 0) {
                        $price = $sku_info['vip_price'];
                    } else {
                        $price = $sku_info["promote_price"];
                    }
                    $update_data = array(
                    ...
                    // 更新数据
                    $box->save($update_data, [
                        "box_id" => $v["box_id"]
                    ]);
                    $box_goods_list[$k]["price"]         = $price;
                    $box_goods_list[$k]["goods_name"]    = $goods_info["goods_name"];
                    $box_goods_list[$k]["sku_name"]      = $sku_info["sku_name"];
                    $box_goods_list[$k]["goods_picture"] = $goods_info["picture"];
                } else {
                    $box_goods_list[$k]["price"]         = $sku_info["promote_price"];
                    $box_goods_list[$k]["goods_name"]    = $goods_info["goods_name"];
                    $box_goods_list[$k]["sku_name"]      = $sku_info["sku_name"];
                    $box_goods_list[$k]["goods_picture"] = $goods_info["picture"];
                }

                $box_goods_list[$k]['stock']               = $sku_info['stock'];
                $box_goods_list[$k]['max_buy']             = $goods_info['max_buy'];
                $box_goods_list[$k]['min_buy']             = $goods_info['min_buy'];
                $box_goods_list[$k]['point_exchange_type'] = $goods_info['point_exchange_type'];
                $box_goods_list[$k]['point_exchange']      = $goods_info['point_exchange'];
                $box_goods_list[$k]['source_type']         = $goods_info['source_type'];

                if ($goods_info['state'] != 1) {
                    unset($box_goods_list[$k]);
                    // 更新cookie购物车
                    $this->cartDelete($v['box_id']);
                    continue;
                }
                $num = $v['num'];
                if ($goods_info['max_buy'] != 0 && $goods_info['max_buy'] < $v['num']) {
                ...
                if ($num != $v['num']) {
                    // 更新购物车
                    $box_goods_list[$k]['num'] = $num;
                    $this->cartAdjustNum($v['box_id'], $num);
                }

                $box_goods_list[$k]["promotion_price"] = $box_goods_list[$k]["price"];
                // 阶梯优惠后的价格
                $box_goods_list[$k]["price"] = $this->getGoodsLadderPreferentialInfo($v['goods_id'], $num, $box_goods_list[$k]["price"]);

                // 为购物车图片
//                $picture = new AlbumPictureModel();
//                $picture_info = $picture->get($v['goods_picture']);
//                $box_goods_list[$k]['picture_info'] = $picture_info;
            ...
            if (!empty($box_goods_list)) {
                sort($box_goods_list);
                $box_goods_list[0]["goods_id_array"] = $goods_id_array;
            }
        }
        return $box_goods_list;
    }

    public function materialCode($goods_array)
    {
        $skuModel    = new NsGoodsSkuModel();
        $update_data = array(
            "material_code" => $goods_array["material_code"]
        );
        $skuModel->save($update_data, [
            "goods_id" => $goods_array["goods_id"],
            "code" => $goods_array["code"]
        ]);
        return 1;
    }

    public function codeUpdate($goods_sku_array)
    {
        $orderGoodsModel = new NsOrderGoodsModel();
        $update_data     = array(
            "code" => $goods_sku_array["code"],
            "material_code" => $goods_sku_array["material_code"]
        );
        $orderGoodsModel->save($update_data, [
            "sku_id" => $goods_sku_array["sku_id"]
        ]);
        return 1;
    }

    //获取购物车所有信息
    public function cartDataImport($cart_array)
    {
        $userService  = new User();
        $userDetail   = $userService->getUserInfoByUid($cart_array['buyer_id']);
        $user_open_id = $userDetail['wx_openid'];

        $goodsModel                    = new NsGoodsModel();
        $goodsDetail                   = $goodsModel->get($cart_array['goods_id']);
        $sku_product_list['item_code'] = $goodsDetail['goods_id'];
        $sku_product_list['title']     = $goodsDetail['goods_name'];
        $sku_product_list['desc']      = $goodsDetail['introduction'];
        ...
        if ($goodsDetail['category_id_1'] != 0) {
            ...
            $sku_product_list['category_list'][] = $categoryDetail['category_name'];
        }
        ...
        if (!empty($goodsDetail['img_id_array'])) {
            $img_id_array = explode(',', $goodsDetail['img_id_array']);
            foreach ($img_id_array as $val) {
                // 查询图片表
                $goods_img                        = new AlbumPictureModel();
                $goodsImgDetail                   = $goods_img->get($val);
                $sku_product_list['image_list'][] = $goodsImgDetail['pic_cover'];
            }
        }

        $sku_product_list['src_wxapp_path'] = "/pages/goods/goodsdetail/goodsdetail?goods_id={$goodsDetail['goods_id']}&goods_name={$goodsDetail['goods_name']}";

        $goodsSkuModel                                  = new NsGoodsSkuModel();
        $goodsSkuDetail                                 = $goodsSkuModel->get($cart_array['sku_id']);
        $sku_product_list['sku_info']['sku_id']         = $goodsSkuDetail['sku_id'];
        $sku_product_list['sku_info']['price']          = $goodsSkuDetail['promote_price'] * 100;
        $sku_product_list['sku_info']['original_price'] = $goodsSkuDetail['price'] * 100;
        if ($goodsDetail['state'] == 1) {
            $sku_product_list['sku_info']['status'] = $goodsDetail['state'];
        } else {
            $sku_product_list['sku_info']['status'] = 2;
        }
    ...
    public function getCart($uid, $shop_id = 0)
    {
        ...
        if (!empty($cart_goods_list)) {
            foreach ($cart_goods_list as $k => $v) {
                $goods      = new NsGoodsModel();
                $goods_info = $goods->getInfo([
                    'goods_id' => $v['goods_id'], 'stock' => ['GT', 0]
                ], 'max_buy,state,point_exchange_type,point_exchange,goods_name,price, vip_price, picture, min_buy, source_type, brand_id, sale_type, sale_end_time, delivery_end_time');
                // 获取商品sku信息
                $goods_sku = new NsGoodsSkuModel();
                $sku_info  = $goods_sku->getInfo([
                    'sku_id' => $v['sku_id'], 'stock' => ['GT', 0]
                ], 'stock, price, vip_price, sku_name, promote_price');
                        ...
                        if ($promotion_info) {
                            ...
                            if ($promotion_goods_info) {
                                ...
                                $n_price = $promotion_goods_info['n_price'] == 0 ?
                                    ...
                                    $promotion_goods_info['n_price'];
                                $cart_goods_list[$k]["interior_price"] = sprintf("%.2f", $n_price);
//                                $cart_goods_list[$k]["stock"]          = $promotion_goods_info['sku_num'] - $promotion_goods_info['use_num'];
                            }
                        }
                    }
...
//                    }
                    ...
                    $update_data = array(
                        "goods_name" => $goods_info["goods_name"],
                        "sku_name" => $sku_info["sku_name"],
                        "goods_picture" => $goods_info["picture"], // $v['goods_picture'],
                        "price" => $price
                    );
                    // 更新数据
                    ...
                    $cart_goods_list[$k]["goods_picture"] = $goods_info["picture"]; // $v['goods_picture'];
                    $cart_goods_list[$k]["is_inside"]     = $is_inside;
                } else {
                    $cart_goods_list[$k]["price"]         = $sku_info["promote_price"];
                    $cart_goods_list[$k]["goods_name"]    = $goods_info["goods_name"];
                    $cart_goods_list[$k]["sku_name"]      = $sku_info["sku_name"];
                    $cart_goods_list[$k]["goods_picture"] = $goods_info["picture"]; // $v['goods_picture'];
                    $cart_goods_list[$k]["is_inside"]     = 0;
                }

                //通过商品的brand_id是否可以改价
                if ($goods_info['brand_id'] == 2) {
                    $cart_goods_list[$k]['is_change_price']  = 1;
                    $cart_goods_list[$k]['min_change_price'] = round($sku_info['price'] * 0.9);
                } else if ($goods_info['brand_id'] == 11) {
                    $cart_goods_list[$k]['is_change_price']  = 1;
                    $cart_goods_list[$k]['min_change_price'] = round($sku_info['price'] * 0.8);
                } else {
                    $cart_goods_list[$k]['is_change_price'] = 0;
                }
                ...
                $cart_goods_list[$k]['source_type']         = $goods_info['source_type'];
                $cart_goods_list[$k]['sale_type']         = $goods_info['sale_type'];
                $cart_goods_list[$k]['sale_end_time']         = $goods_info['sale_end_time'];
                $cart_goods_list[$k]['delivery_end_time']         = $goods_info['delivery_end_time'];
...
//                $cart_goods_list[$k]['picture_info'] = $picture_info;
     ...
     * 添加礼品盒(non-PHPdoc)
     *
     * @see \data\api\IGoods::addBox()
     */
    public function addBox($uid, $shop_id, $shop_name, $goods_id, $goods_name, $sku_id, $sku_name, $price, $num, $picture)
    {
        $retval = array(
            'code' => 0,
            "message" => ""
        ...
        if ($uid > 0) {
            ...
            $count = $box->where($condition)->count();
            if ($count == 0 || empty($count)) {
                $data = array(
                    'buyer_id' => $uid,
                    'shop_id' => $shop_id,
                ...
                $box->save($data);
                $retval['code']    = $box->box_id;
                $retval['message'] = lang("added_box_success");
            } else {
                $box = new NsBoxModel();
                // 查询商品限购
                $goods   = new NsGoodsModel();
                $get_num = $box->getInfo($condition, 'box_id,num');
                $max_buy = $goods->getInfo([
                    'goods_id' => $goods_id
                ], 'max_buy');
                $new_num = $num + $get_num['num'];
                ...
                $res  = $box->save($data, $condition);
                if ($res) {
                    $retval['code']    = $get_num['box_id'];
                    $retval['message'] = lang("added_box_success");
                }
            }
        } else {
            ...
            if (!empty($box_array)) {
                $box_array = json_decode($box_array, true);
                $tmp_array = array();
                foreach ($box_array as $k => $v) {
                    $tmp_array[] = $v['box_id'];
                }
                $box_id  = max($tmp_array) + 1;
                $is_have = true;
                foreach ($box_array as $k => $v) {
                    if ($v["goods_id"] == $goods_id && $v["sku_id"] == $sku_id) {
                        $is_have              = false;
                        $box_array[$k]["num"] = $data["num"] + $v["num"];
                    }
                }
            ...
            try {
                ...
                if ($goods_purchase_restriction['code'] == 0) {
                    ...
                    $box_array_string = json_encode($box_array);
                    cookie('box_array', $box_array_string, 3600);
                    $retval['code']    = 1;
                    $retval['message'] = lang("added_box_success");
                }
            } catch (\Exception $e) {
                ...
                $retval['code']    = 0;
                $retval['message'] = lang("failed_to_add_box");
            }
        }
        return $retval;
    }
     ...
     * 礼品盒数量修改(non-PHPdoc)
     *
     * @see \data\api\IGoods::boxAdjustNum()
     */
    public function boxAdjustNum($box_id, $num)
    {
        if ($this->uid > 0) {
            $box    = new NsBoxModel();
            $data   = array(
            ...
            $retval = $box->save($data, [
                'box_id' => $box_id
            ]);
            return $retval;
        } else {
            $result = $this->updateCookieBoxNum($box_id, $num);
            return $result;
        }
    }
     ...
     * 礼品盒项目删除(non-PHPdoc)
     *
     * @see \data\api\IGoods::boxDelete()
     */
    public function boxDelete($box_id_array)
    {
        if ($this->uid > 0) {
            $box    = new NsBoxModel();
            $retval = $box->destroy($box_id_array);
            return $retval;
        } else {
            $result = $this->deleteCookieBox($box_id_array);
            return $result;
        }
    }
    ...
    public function setEvaluteCheckAjax($id)
    {
        $goodsEvaluate = new NsGoodsEvaluateModel();
        $showStatu     = $goodsEvaluate->getInfo([
            'id' => $id
        ], 'is_check');
        if ($showStatu['is_check'] == 1) {
            return $goodsEvaluate->save([
                'is_check' => 0
            ], [
                'id' => $id
            ]);
        } elseif ($showStatu['is_check'] == 0) {
            return $goodsEvaluate->save([
                'is_check' => 1
            ], [
                'id' => $id
    ...
    public function setEvaluteFirstAjax($id)
    {
        $goodsEvaluate = new NsGoodsEvaluateModel();
        $showStatu     = $goodsEvaluate->getInfo([
            'id' => $id
        ], 'is_first');
        if ($showStatu['is_first'] == 1) {
            return $goodsEvaluate->save([
                'is_first' => 0
            ], [
                'id' => $id
            ]);
        } elseif ($showStatu['is_first'] == 0) {
            return $goodsEvaluate->save([
                'is_first' => 1
            ], [
                'id' => $id
    ...
    public function copyGoodsInfo($goods_id)
    {
        ...
        $res = $this->addOrEditGoods(0, $goods_detail['goods_name'] . '-副本', $goods_detail['shop_id'], $goods_detail['category_id'], $goods_detail['category_id_1'], $goods_detail['category_id_2'], $goods_detail['category_id_3'], $goods_detail['supplier_id'], $goods_detail['brand_id'], $goods_detail['group_id_array'], $goods_detail['source_type'], $goods_detail['goods_type'], $goods_detail['market_price'], $goods_detail['price'], $goods_detail['vip_price'], $goods_detail['point_exchange_type'], $goods_detail['point_exchange'], $goods_detail['give_point'], $goods_detail['is_member_discount'], $goods_detail['shipping_fee'], $goods_detail['shipping_fee_id'], $goods_detail['stock'], $goods_detail['max_buy'], $goods_detail['min_buy'], $goods_detail['min_stock_alarm'], $goods_detail['sales'], $goods_detail['collects'], $goods_detail['star'], $goods_detail['evaluates'], $goods_detail['province_id'], $goods_detail['city_id'], $goods_detail['picture'], $goods_detail['keywords'], $goods_detail['introduction'], $goods_detail['description'], $goods_detail['QRcode'], $goods_detail['is_vip'], $goods_detail['is_black_tech'], $goods_detail['material_black'], $goods_detail['use_black'], $goods_detail['effect_black'], $goods_detail['selling_point'], $goods_detail['code'], $goods_detail['material_code'], $goods_detail['fraction'], $goods_detail['is_stock_visible'], $goods_detail['is_hot'], $goods_detail['is_recommend'], $goods_detail['is_new'], $goods_detail['sort'], $goods_detail['img_id_array'], $skuArray, 0, $goods_detail['sku_img_array'], $goods_detail['goods_attribute_id'], json_encode($goods_attribute_arr), $goods_detail['goods_spec_format'], $goods_detail['goods_weight'], $goods_detail['goods_volume'], $goods_detail['shipping_fee_type'], $goods_detail['extend_category_id'], $goods_sku_picture_str, $goods_detail['virtual_goods_type_id'], $goods_detail['production_date'], $goods_detail['shelf_life'], $ladder_preference, $goods_detail['goods_video_address'], $goods_detail['new_pic'], $goods_detail['sale_type'], getTimeStampTurnTime($goods_detail['sale_end_time']), getTimeStampTurnTimeByYmd($goods_detail['delivery_end_time']));
        return $res;
    }

    //批量复制商品
    public function batchCopyGoodsInfo($goods_ids,$category_id1,$name_add_type,$name)
    {
        $goods_id_array = explode(',', $goods_ids);
        foreach ($goods_id_array as $key => $val) {
            $goods_detail        = $this->getGoodsDetail($val);
            ...
            foreach ($goods_detail['sku_list'] as $item) {
                if (!empty($item['attr_value_items'])) {
                    $skuArray .= $item['attr_value_items'] . '¦' . $item['price'] . "¦" . $item['market_price'] . "¦" . $item['cost_price'] . "¦" . $item['stock'] . "¦" . $item['code'] . "¦" . $item['material_code'] . '§';
                }
            }
            $skuArray = rtrim($skuArray, '§');
            ...
            // sku规格图片
            $goods_sku_picture             = new NsGoodsSkuPictureModel();
            $goods_sku_picture_query       = $goods_sku_picture->getQuery(["goods_id" => $val], "goods_id, shop_id, spec_id, spec_value_id, sku_img_array", '');
            $goods_sku_picture_query_array = array();
            foreach ($goods_sku_picture_query as $k => $v) {
                $goods_sku_picture_query_array[$k]["spec_id"]       = $v["spec_id"];
                $goods_sku_picture_query_array[$k]["spec_value_id"] = $v["spec_value_id"];
            ...
            // 阶梯优惠信息
            $goodsLadderPreferentialList = $this->getGoodsLadderPreferential(["goods_id" => $val]);
            $ladder_preference = implode(",", $goodsLadderPreferentialList);

            if($name_add_type == 1){
                $goods_name = $name.$goods_detail['goods_name'];
            }else{
                $goods_name = $goods_detail['goods_name'].$name;
            }
            $res = $this->copyGoods($goods_name, $goods_detail['shop_id'], $category_id1, $category_id1, $goods_detail['supplier_id'], $goods_detail['group_id_array'], $goods_detail['source_type'], $goods_detail['goods_type'], $goods_detail['market_price'], $goods_detail['price'], $goods_detail['vip_price'], $goods_detail['point_exchange_type'], $goods_detail['point_exchange'], $goods_detail['give_point'], $goods_detail['is_member_discount'], $goods_detail['shipping_fee'], $goods_detail['shipping_fee_id'], $goods_detail['stock'], $goods_detail['max_buy'], $goods_detail['min_buy'], $goods_detail['min_stock_alarm'], $goods_detail['sales'], $goods_detail['collects'], $goods_detail['star'], $goods_detail['evaluates'], $goods_detail['province_id'], $goods_detail['city_id'], $goods_detail['picture'], $goods_detail['keywords'], $goods_detail['introduction'], $goods_detail['description'], $goods_detail['QRcode'], $goods_detail['is_vip'], $goods_detail['is_black_tech'], $goods_detail['material_black'], $goods_detail['use_black'], $goods_detail['effect_black'], $goods_detail['selling_point'], $goods_detail['code'], $goods_detail['material_code'], $goods_detail['fraction'], $goods_detail['is_stock_visible'], $goods_detail['is_hot'], $goods_detail['is_recommend'], $goods_detail['is_new'], $goods_detail['sort'], $goods_detail['img_id_array'], $skuArray, 0, $goods_detail['sku_img_array'], $goods_detail['goods_attribute_id'], json_encode($goods_attribute_arr), $goods_detail['goods_spec_format'], $goods_detail['goods_weight'], $goods_detail['goods_volume'], $goods_detail['shipping_fee_type'], $goods_sku_picture_str, $goods_detail['virtual_goods_type_id'], $goods_detail['production_date'], $goods_detail['shelf_life'], $ladder_preference, $goods_detail['goods_video_address'], $goods_detail['new_pic'], $goods_detail['sale_type'], $goods_detail['sale_end_time'], $goods_detail['delivery_end_time']);
            if ($res > 0) {
                $goodsId = $res;
                ...
                $this->goods_code_make($goodsId, $pay_qrcode, $wxcode['url_online'],$wxcode['url_store']);
            }
        }
        return $res;
    }
    ...
    public function getGoodsWxCode($goods_id){
        ...
        $data_bc      = send_post($url, $post_data_bc);
        $code_bc      = data_uri($data_bc, 'image/png');
        $url_online = base64_image_content($code_bc,'upload/goods_wxcode/online',$goods_id);
        ...
        $code_store                   = send_post($url, $post_data_store);
        $code_store = data_uri($code_store, 'image/png');
        $url_store = base64_image_content($code_store,'upload/goods_wxcode/store_1',$goods_id);
        $data = [
    ...
    //商品复制
    public function copyGoods($goods_name, $shopid, $category_id, $category_id_1, $supplier_id, $group_id_array, $source_type, $goods_type, $market_price, $price, $vip_price, $point_exchange_type, $point_exchange, $give_point, $is_member_discount, $shipping_fee, $shipping_fee_id, $stock, $max_buy, $min_buy, $min_stock_alarm, $sales, $collects, $star, $evaluates, $province_id, $city_id, $picture, $keywords, $introduction, $description, $QRcode, $is_vip, $is_black_tech, $material_black, $use_black, $effect_black, $selling_point, $code, $material_code, $fraction, $is_stock_visible, $is_hot, $is_recommend, $is_new, $sort, $image_array, $sku_array, $state, $sku_img_array, $goods_attribute_id, $goods_attribute, $goods_spec_format, $goods_weight, $goods_volume, $shipping_fee_type, $sku_picture_values, $virtual_goods_type_id, $production_date, $shelf_life, $ladder_preference, $goods_video_address, $new_pic, $sale_type, $sale_end_time, $delivery_end_time)
    {
        $error         = 0;
        $goods = new NsGoodsModel();
        $goods->startTrans();
        try {
            ...
            //start
            $goods->save($data_goods);
            $goods_id = $goods->goods_id;
            // 添加sku
            if (!empty($sku_array)) {
                $sku_list_array = explode('§', $sku_array);
                foreach ($sku_list_array as $k => $v) {
                    $res = $this->addOrUpdateGoodsSkuItem($goods->goods_id, $v);
                    if (!$res) {
                        $error = 1;
                    }
                }
    ...
    /*
     * (non-PHPdoc)
     * @see \data\api\IGoods::deleteCookieBag()删除cookie礼品盒
     */
    private function deleteCookieBox($box_id_array)
    {
        // TODO Auto-generated method stub
        // 获取删除条件拼装
        $box_id_array = trim($box_id_array);
        if (empty($box_id_array) && $box_id_array != 0) {
            return 0;
        }
        // 获取礼品盒
        $box_goods_list = cookie('box_array');
        ...
        foreach ($box_goods_list as $k => $v) {
            if (strpos((string)$box_id_array, (string)$v["box_id"]) !== false) {
                unset($box_goods_list[$k]);
            }
        }
        if (empty($box_goods_list)) {
            cookie('box_array', null);
            return 1;
        } else {
            sort($box_goods_list);
            try {
                cookie('box_array', json_encode($box_goods_list), 3600);
                return 1;
            } catch (\Exception $e) {
                return 0;
            }
     ...
     * @return number
     */
    private function updateCookieBoxNum($box_id, $num)
    {
        // 获取礼品盒
        $box_goods_list = cookie('box_array');
        if (empty($box_goods_list)) {
    ...
    public function getGoodsCoupon($goods_id, $uid)
    {
        ...
        foreach ($coupon_goods_type_id_list as $k => $v) {
            ...
            ], "start_time,end_time,is_show_detail");
            if ($res['start_time'] > time() || time() > $res['end_time'] || $res['is_show_detail'] != 1) {
    ...
    function getGoodsPurchaseRestrictionForCurrentUser($goods_id, $num = 0, $flag = "")
    {
        ...
        $result = $num; // 用户购买的数量 + 购物车中的数量 + 礼包中的数量 + 订单交易数量 不能超过商品的限购
     ...
     */
    public function getGoodsQueryLimit($condition, $field, $page_size = PAGESIZE, $order = "ng.create_time desc")
    {
        $goods_model = new NsGoodsModel();
        $list        = $goods_model->alias("ng")
            ->join('sys_album_picture ng_sap', 'ng_sap.pic_id = ng.picture', 'left')
            ->join('ns_goods_category ngc', 'ngc.category_id = ng.category_id', 'left')
            ->field($field)
            ->where($condition)
            ->order($order)
            ->limit("0,$page_size")
    ...
    //根据条件查询商品列表
    public function getBlackTechGoodsQuery($condition, $field, $order = "ng.price asc")
    {
        $goods_model = new NsGoodsModel();
        $list        = $goods_model->alias("ng")
            ->join('sys_album_picture ng_sap', 'ng_sap.pic_id = ng.picture', 'left')
    ...
    //极选师商品推荐排行榜
    public function getGoodsRecommendList($condition, $field, $order = "ng.price asc")
    {
        $goods_model = new NsGoodsModel();
        $list        = $goods_model->alias("ng")
            ->join('sys_album_picture ng_sap', 'ng_sap.pic_id = ng.picture', 'left')
    ...
    public function getBackStageGoodsList($page_index = 1, $page_size = 0, $condition = '', $order = 'ng.sort asc')
    {
        ...
        $viewObj = $goods_model->alias("ng")
            ->join('sys_album_picture ng_sap', 'ng_sap.pic_id = ng.picture', 'left')
            ->field("ng.goods_id,ng.source_type,ng.goods_name,ng.market_price,ng.price,ng.promotion_price,ng.vip_price,ng.goods_type,ng.stock,ng.introduction,ng.max_buy,ng.state,ng.is_hot,ng.is_recommend,ng.is_new,ng.sales,ng_sap.pic_cover_micro,ng.is_vip,ng.is_black_tech,ng.code,ng.material_code,ng.create_time,ng.QRcode,ng.real_sales,ng.sort,ng.group_id_array,ng.fraction,ng.sale_type,ng.sale_end_time,ng.delivery_end_time");
    ...
    public function getGoodsListNew($page_index = 1, $page_size = 0, $condition = '', $order = 'ng.sort asc')
    {
        ...
        $viewObj          = $goods_model->alias("ng")
            ->join('sys_album_picture ng_sap', 'ng_sap.pic_id = ng.picture', 'left')
            ->field("ng.goods_id,ng.goods_name,ng_sap.pic_cover_mid,ng.promotion_price,ng.market_price,ng.goods_type,ng.stock,ng_sap.pic_id,ng.max_buy,ng.state,ng.is_hot,ng.is_recommend,ng.is_new,ng.sales,ng_sap.pic_cover_small,ng.group_id_array,ng.shipping_fee");
     ...
     * 内购商品
     */
    public function getNeiGouGoodsList($page_index, $page_size, $condition = '', $order = 'ng.sort asc')
    {
        $neigou_model     = new NsPromotionNeigouGoodsModel();
        $goods_view_model = new NsGoodsViewModel();
        // 针对商品分类

        $viewObj = $neigou_model->alias('ngg')
            ->join('ns_goods ng', 'ng.goods_id = ngg.goods_id', 'left')
            ->join('ns_goods_category ngc', 'ngc.category_id = ng.category_id', 'left')
            ->join('sys_album_picture sap', 'ng.picture = sap.pic_id', 'left')
            ->field('ng.goods_id, ng.goods_name, ng.source_type, ng.material_code, ng.shop_id, ng.category_id, ng.brand_id, ng.group_id_array,
             ng.promotion_type, ng.goods_type, ng.market_price, ng.price, ng.promotion_price,
            ng.cost_price, ng.vip_price, ng.point_exchange_type, ng.point_exchange, ng.give_point,
            ...
            ng.update_time, ng.sort, ng.real_sales, ngc.category_id, ngc.category_name, sap.pic_cover_micro,sap.pic_cover_mid,sap.pic_cover_small,sap.pic_id,ngg.n_price as inside_price,ngg.n_discount,ngg.sku_id')
            ->group('goods_id');
        $queryList  = $neigou_model->viewPageQuery($viewObj, $page_index, $page_size, $condition, $order);
        $queryCount = count($queryList);
        $list       = $goods_view_model->setReturnList($queryList, $queryCount, $page_size);
        return $list;
    }
     ...
     * @return boolean
     */
    public function updateGoodsStock($goods_id, $stock)
    {
        $goods = new NsGoodsModel();
        return $goods->save([
            'stock' => $stock
     ...
     * @return boolean
     */
    public function updateGoodsSkuStock($goods_id, $stock)
    {
    ...
    /**
     * 根据条件、指定数量查询商品 首页底部分页数据
     * 创建时间：2018年1月3日15:06:51
     * (non-PHPdoc)
     *
     * @see \data\api\IGoods::getRecommendGoodsQuery()
     */
    public function getGoodsQueryLimitPage($condition = '',$field, $page_index, $page_size,  $order = 'ng.sort asc')
    {
        $goods_model = new NsGoodsModel();
        $viewObj     = $goods_model->alias("ng")
            ->join('sys_album_picture ng_sap', 'ng_sap.pic_id = ng.picture', 'left')
            ->join('ns_goods_category ngc', 'ngc.category_id = ng.category_id', 'left')
            ->field($field);
        $queryList  = $goods_model->viewPageQuery_1($viewObj, $page_index, $page_size, $condition, $order);
        $queryCount = count($queryList);
        $list       = $goods_model->setReturnList($queryList, $queryCount, $page_size);
        return $list;
    }
    ...
    //同分类,同品牌相似商品推荐
    public function similarGoodsList($goods_id,$category_id_1,$brand_id)
    {
        $goods_model = new NsGoodsModel();
        $list['similarOne'] = $goods_model->goodsList(['ng.state'=>1,'ng.goods_id'=>['neq',$goods_id],'ng.category_id_1'=>$category_id_1, 'ng.brand_id'=>$brand_id]);
        $list['similarTwo'] = $goods_model->goodsList(['ng.state'=>1, 'ng.category_id_1'=>$category_id_1, 'ng.brand_id'=>['neq',$brand_id]]);
        $list['similarThree'] = $goods_model->goodsList(['ng.state'=>1, 'ng.category_id_1'=>['neq',$category_id_1], 'ng.brand_id'=>$brand_id]);
        return $list;
    }
}
