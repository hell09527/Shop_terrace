...
class MemberCoupon extends BaseService
{
    ...
    {
        // $get_type 1.订单 2.首页领取 3 4.营销活动获取 5.分享获取
        $coupon      = new NsCouponModel();
...
//                );
    ...
    public function getCouponTypeList()
    {
        ...
        $coupon_type_model = new NsCouponTypeModel();
        $coupon_type_list  = $coupon_type_model->getQuery($condition, '*', 'create_time desc');
        $coupon_arr        = [];
        if (!empty($coupon_type_list)) {
            $coupon_model = new NsCouponModel();
            $i = 0;
            foreach ($coupon_type_list as $k => $v) {
                $coupon_info = $coupon_model->getQuery([
                    'coupon_type_id' => $v['coupon_type_id'],
                    'state'          => 0,
                    'uid'            => 0
                ], '*', '');
                if (count($coupon_info) > 0) {
                    $coupon_arr[$i]['coupon_id']      = $coupon_info[0]['coupon_id'];
                    $coupon_arr[$i]['coupon_code']    = $coupon_info[0]['coupon_code'];
                    $coupon_arr[$i]['money']          = $coupon_info[0]['money'];
                    $coupon_arr[$i]['use_type']       = $coupon_info[0]['use_type'];
                    $coupon_arr[$i]['get_after_days'] = $coupon_info[0]['get_after_days'];
                    $coupon_arr[$i]['pay_money_get']  = $coupon_info[0]['pay_money_get'];
                    $coupon_arr[$i]['start_time']     = $coupon_info[0]['start_time'];
                    $coupon_arr[$i]['end_time']       = $coupon_info[0]['end_time'];
                    $coupon_arr[$i]['coupon_type_id'] = $v['coupon_type_id'];
                    $coupon_arr[$i]['coupon_name']    = $v['coupon_name'];
                    $coupon_arr[$i]['at_least']       = $v['at_least'];
                } else {
                    continue;
                }
                $i ++;
     ...
     * 分享领取优惠券
     */
    public function getAchieveCoupon($uid, $coupon_type_id, $get_type,$origin_uid)
    {
        // $get_type 1.订单 2.首页领取 3 4.营销活动获取 5.分享获取
        $coupon      = new NsCouponModel();
        $coupon_type = new NsCouponTypeModel();
