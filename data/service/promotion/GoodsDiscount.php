...
class GoodsDiscount extends BaseService
{
     ...
     */
    public function getGoodsIsDiscount($sku_id, $start_time, $end_time)
    {
    ...
    public function getDiscountGoodsList($page_index = 1, $page_size = 0, $condition = array(), $order = '')
    {
        ...
        $goods_list = $discount_goods->getViewList($page_index, $page_size, $condition, $order); # 添加查询字段picture
        if (! empty($goods_list['data'])) {
            ...
            foreach ($goods_list['data'] as $k => $v) {
                ...
                $picture_info = $picture->get($v['picture']);  # todo @陈 修改goods_picture字段为picture
