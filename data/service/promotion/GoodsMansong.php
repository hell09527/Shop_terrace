...
class GoodsMansong extends BaseService
{
     ...
     */
    public function getGoodsIsMansong($sku_id, $start_time, $end_time)
    {
...
//                            $goods_sku_promote_price = $rule['discount'] * $goods_sku_price / $goods_sku_list_price;
                            $rule_rate_discount      = $rule['discount_rate'] == 0 ? 0 : (10 - $rule['discount_rate']) / 10 * $goods_sku_list_price;
                            $rule_discount           = max($rule_rate_discount, $rule['discount']);
                            $goods_sku_promote_price = $rule_discount * $goods_sku_price / $goods_sku_list_price;
     ...
     */
    public function getShareGoodsSkuListMansong($share_list)
    {

        $discount_info        = array();
        $goods_preference     = new GoodsPreference();
        $goods_sku_list_price = $goods_preference->getShareGoodsSkuListPrice($share_list);
        ...
        if (!empty($share_list)) {
            ...
            if (!empty($list_quan[0])) {    //存在全场满减送
                $goods_sku_list_array = explode(",", $share_list);
                $rule_list            = $this->getMansongRule($list_quan[0]['mansong_id']);//获取满减规则
                $discount             = array();
                //获取订单项减现金额
                foreach ($rule_list as $k_rule => $rule) {
                    if ($rule['price'] <= $goods_sku_list_price) {
                        foreach ($goods_sku_list_array as $k_goods_sku => $v_goods_sku) {
                            $sku_data_goods          = explode(":", $v_goods_sku);
                            $sku_id_goods            = $sku_data_goods[0];//sku_id
                            $goods_preference        = new GoodsPreference();
                            $goods_sku_price         = $goods_preference->getShareGoodsSkuListPrice($v_goods_sku);//订单项总价
                            $rule_rate_discount      = $rule['discount_rate'] == 0 ? 0 : (10 - $rule['discount_rate']) / 10 * $goods_sku_list_price;//订单项折扣可优惠金额
                            $rule_discount           = max($rule_rate_discount, $rule['discount']);
                            $goods_sku_promote_price = $rule_discount * $goods_sku_price / $goods_sku_list_price;//订单项实际可优惠金额
                            $discount[]              = array($rule, $sku_id_goods . ":" . $goods_sku_promote_price);
                        }
                        break;
                    }
            ...
            } else {    //存在部分商品满减送活动(只可能存在部分商品满减送)

                //1.查询商品列表可能的满减送活动列表
                $mansong_list = $this->getShareGoodsSkuMansongList($share_list);
                if (!empty($mansong_list)) {
                    //循环满减送活动
                    foreach ($mansong_list as $k => $v) {
                        $discount_info_detail = $this->getShareMansongGoodsSkuListPromotion($v, $share_list);
                        $discount_info[]      = $discount_info_detail;
                    }
                }
            }
     ...
     * @param unknown $goods_sku_list
     */
    public function getShareGoodsMansongMoney($share_list)
    {
        $mansong_array   = $this->getShareGoodsSkuListMansong($share_list);
        $promotion_money = 0.00;
        if (!empty($mansong_array)) {
            foreach ($mansong_array as $k_mansong => $v_mansong) {
                foreach ($v_mansong['discount_detail'] as $k_rule => $v_rule) {
     ...
     */
    public function getGoodsMansongPromotion($sku_id)
    {
        $time = date("Y-m-d H:i:s", time());
        ...
        if ($member_info == 1) {
            if ($promotion_info) {
                $promotion_goods_info = \think\Db::name('ns_promotion_neigou_goods')->where(['discount_id' => $promotion_info['mansong_id'], 'sku_id' => $sku_id])->find();
        ...
        foreach ($list as $k => $v) {
            ...
            $info                    = $promotion_mansong_goods->getInfo(['mansong_id' => $v['mansong_id'], 'sku_id' => $sku_id], '*');
            if (!empty($info)) {
                return $v;
            }
    ...
    public function getGoodsSkuMansongList($goods_sku_list)
    {
        ...
        if (!empty($goods_sku_list)) {
            ...
            foreach ($goods_sku_list_array as $k => $v) {
                ...
                $promotion = $this->getGoodsMansongPromotion($sku_id);
                if (!empty($promotion)) {
    ...
    /**
     * 清单分享:获取商品sku的满减送活动列表
     * @param unknown $goods_sku_list
     */
    public function getShareGoodsSkuMansongList($share_list)
    {
        $promotion_array = array();
        if (!empty($share_list)) {
            $goods_sku_list_array = explode(",", $share_list);
    ...
    public function getMansongRuleNew($mansong_id)
    {
        $mansong_rule = new NsPromotionMansongRuleModel();
        $rule_list    = $mansong_rule->getQuery(['mansong_id' => $mansong_id], '*', 'price asc');
        return $rule_list;
    }
     ...
     * @param unknown $goods_id
     */
    public function getGoodsMansongDetail($sku_id)
    {
        //查询全场满减送活动
        //检测店铺是否存在正在进行的全场满减送活动
        $condition         = array(
        ...
        if (empty($mansong_promotion)) {
            $mansong_promotion = $this->getGoodsMansongPromotion($sku_id);
        }

        if (!empty($mansong_promotion)) {
            $rule                      = $this->getMansongRuleNew($mansong_promotion['mansong_id']);
     ...
     * @param unknown $goods_id
     */
    public function getGoodsMansongName($sku_id)
    {
        //查询满减送活动详情
        $mansong_detail = $this->getGoodsMansongDetail($sku_id);
        $mansong_name   = '';
        if (!empty($mansong_detail)) {
            foreach ($mansong_detail['rule'] as $k => $v) {
                // $mansong_name .= '满'.$v['price'].'减'.$v['discount'].' ';
                $mansong_name .= '满' . $v['price'];
                if ($v['discount'] > 0) {
                    $mansong_name .= '减' . $v['discount'] . ' ';
                }
                if ($v['discount_rate'] > 0) {
                    $mansong_name .= '打' . $v['discount_rate'] . '折' .' ';
                }
...
//                if ($v['gift_id'] != 0) {
//                    $v_arr     = explode(':',$v['gift_id']);
//                    $gift      = new NsPromotionGiftModel();
//                    $gift_name = $gift->getInfo(['gift_id' => $v_arr[0]], 'gift_name');
//                    $mansong_name .= '赠送' . $gift_name['gift_name'].'x' . $v_arr[1] . '件';
//
//                }
                ...
                if ($v['gift_id'] != 0) {
                    ...
                    if (strpos($v['gift_id'], ',') !== false) {
                        ...
                        foreach ($ids as $key => $vo) {
                            $_ids      = explode(':', $vo);
                            $giftInfo = $gift->getInfo([
                                'gift_id' => $_ids[0]
                            ], 'gift_name');
                            $gift_name .= $giftInfo['gift_name'] . ' x ' . $_ids[1] . '件' . ',';
                        }
                        $gift_name = substr($gift_name, 0, -1);
                    } else {
                        $_ids       = explode(':', $v['gift_id']);
                        $giftInfo  = $gift->getInfo([
                            'gift_id' => $_ids[0]
                        ], 'gift_name');
                        $gift_name = '赠送'.$giftInfo['gift_name'] . ' x ' . $_ids[1] . '件';
                    }

                    $mansong_name .= $gift_name;
                }
     ...
     * @param unknown $goods_id
     */
    public function getGoodsMansongNameArray($sku_id)
    {
        //查询满减送活动详情
        $mansong_detail = $this->getGoodsMansongDetail($sku_id);
        $mansong_name_array   = [];
        if (!empty($mansong_detail)) {
            foreach ($mansong_detail['rule'] as $k => $v) {
                $mansong_name_array[$k] .= '满' . $v['price'];
                if ($v['discount'] > 0) {
                    $mansong_name_array[$k] .= '减' . $v['discount'] . ' ';
                }
                if ($v['discount_rate'] > 0) {
                    $mansong_name_array[$k] .= '打' . $v['discount_rate'] . '折' .' ';
                }
                if ($v['free_shipping'] == 1) {
                    $mansong_name_array[$k] .= '免邮' . ' ';
                }
                if ($v['give_point'] != 0) {
                    $mansong_name_array[$k] .= '赠送' . $v['give_point'] . '积分' . ' ';
                }
                if ($v['give_coupon'] != 0) {
                    $coupon      = new NsCouponModel();
                    $coupon_name = $coupon->getInfo(['coupon_type_id' => $v['give_coupon']], 'money');
                    $mansong_name_array .= '赠送' . $coupon_name['money'] . '元优惠券' . ' ';
                }
                ...
                if ($v['gift_id'] != 0) {
                    ...
                    $mansong_name_array[$k] .= $gift_name;
                }
            }
        }
        return $mansong_name_array;
    }
    ...
    public function getMansongGoodsSkuListPromotion($mansong_obj, $goods_sku_list)
    {
        ...
        foreach ($goods_sku_list_array as $k => $v) {
            foreach ($mansong_goods as $k_mansong => $v_mansong) {
                # @戴 新增规格后修改  goods_id -> sku_id
                if ($v[0] == $v_mansong['sku_id']) {
...
//                        $goods_sku_promote_price = $rule['discount'] * $goods_sku_price / $new_sku_list_price;
                        ...
                        $rule_rate_discount      = $rule['discount_rate'] == 0 ? 0 : (10 - $rule['discount_rate']) / 10 * $new_sku_list_price;
                        $rule_discount           = max($rule_rate_discount, $rule['discount']);
                        $goods_sku_promote_price = $rule_discount * $goods_sku_price / $new_sku_list_price;
                        $discount[]              = array($rule, $sku_id_goods . ":" . $goods_sku_promote_price);
    ...
    /**
     * 清单分享:查询对应满减送活动的商品列表的优惠情况
     * @param unknown $mansong_obj (只针对部分商品满减)
     * @param unknown $share_list
     */
    public function getShareMansongGoodsSkuListPromotion($mansong_obj, $share_list)
    {
        //查询组装新的sku列表
        $mansong_goods = $this->getMansongGoods($mansong_obj['mansong_id']);

        $new_share_list         = '';
        $share_list_array = explode(",", $share_list);
        foreach ($share_list_array as $k => $v) {
            $info_array  = explode(":", $v);
            foreach ($mansong_goods as $k_mansong => $v_mansong) {
                if ($info_array[0] == $v_mansong['sku_id']) {
                    $new_share_list         = $new_share_list . $info_array[0] . ':' . $info_array[1] . ':' . $info_array[2] . ',';
                }
            }
        }

        if (!empty($new_share_list)) {
            $new_share_list = substr($new_share_list, 0, strlen($new_share_list) - 1);
            $new_share_list_array = explode(",", $new_share_list);

            //获取总价
            $goods_preference   = new GoodsPreference();
            $new_sku_list_price = $goods_preference->getShareGoodsSkuListPrice($new_share_list);
            $rule_list          = $this->getMansongRule($mansong_obj['mansong_id']);//得到满减规则
            $discount           = array();
            //获取订单项减现金额
            foreach ($rule_list as $k_rule => $rule) {
                if ($rule['price'] <= $new_sku_list_price) {
                    foreach ($new_share_list_array as $k_sku => $v_sku) {
                        $info_array         = explode(":", $v_sku);
                        $sku_id_goods            = $info_array[0];
                        $goods_preference        = new GoodsPreference();
                        $goods_sku_price         = $goods_preference->getShareGoodsSkuListPrice($v_sku);//订单项总价
                        $rule_rate_discount      = $rule['discount_rate'] == 0 ? 0 : (10 - $rule['discount_rate']) / 10 * $new_sku_list_price;
                        $rule_discount           = max($rule_rate_discount, $rule['discount']);
                        $goods_sku_promote_price = $rule_discount * $goods_sku_price / $new_sku_list_price;
                        $discount[]              = array($rule, $sku_id_goods . ":" . $goods_sku_promote_price);
