...
use think\Cookie;
use think\Session;
use data\model\NsKolModel as NsKolModel;
use data\model\BcDistributorModel as BcDistributorModel;
...
class Member extends User implements IMember
{
     ...
     */
    public function registerMember($user_name, $password, $email, $mobile, $user_qq_id, $qq_info, $wx_openid, $wx_info, $wx_unionid, $source_branch, $traffic_acquisition_source)
    {
        // if (! empty($user_name)) {
        ...
        if ($res > 0) {
            ...
            } else if (!empty($user_qq_id)) {
                $this->qqLogin($user_qq_id);
            } else if (!empty($wx_openid)) {
                $this->wchatLogin($wx_openid);
            } else if (!empty($wx_unionid)) {
    ...
    public function getMemberList($page_index = 1, $page_size = 0, $condition = '', $order = '', $field = '*')
    {
        ...
        foreach ($result['data'] as $k => $v) {
            if ($v['birthday']) {
                $result['data'][$k]['birthday'] = date('Y-m-d', $v['birthday']);
            }
            //查询来源网点
...
//            }else{
//                $result['data'][$k]['source_branch_name'] = 'BC线上';
//            }

            //查询分销来源
            if ($v['source_distribution'] > 0) {
                $member_detail                                  = $this->getMember($v['source_distribution']);
                $result['data'][$k]['source_distribution_name'] = $member_detail['real_name'];
            } else {
                $result['data'][$k]['source_distribution_name'] = '';
            }
            $member_account                = new MemberAccount();
            $result['data'][$k]['point']   = $member_account->getMemberPoint($v['uid'], '');  //获取会员积分
            $result['data'][$k]['balance'] = $member_account->getMemberBalance($v['uid']);  //获取会员余额数
            $result['data'][$k]['coin']    = $member_account->getMemberCoin($v['uid']);    //获取会员购物币
        }
        return $result;
    }

    //员工列表
    public function getEmployeeList($page_index = 1, $page_size = 0, $condition = '', $order = '')
    {
        $employ_view = new BcEmployeeModel();
        $result      = $employ_view->getViewList($page_index, $page_size, $condition, $order);
        return $result;
    }
    ...
    //会员
    public function memberList($condition, $field, $order)
    {
        $member = new NsMemberModel();
        return $member->getQuery($condition, $field, $order);
    }
    ...
    public function getMember($uid)
    {
        ...
        $member_detail = $member_model->get($uid);
    ...
    public function getMemberDetail($shop_id = '')
    {
        ...
        if (!empty($this->uid)) {
            ...
            //计算付费会员过期时间
            if ($member_info['vip_buy_time']) {
                $member_info['vip_overdue_time'] = $member_info['vip_buy_time'] + 86400 * 365;  //付费会员购买一年过期
            }
            ...
            //是否是内部员工(0:否;1:是)
            $member_info['is_employee'] = $this->getMemberIsEmployee();
    ...
    //查询用户是否是内部员工
    public function getMemberIsEmployee()
    {
        if (!empty($this->uid)) {
            $user       = new UserModel();
            $userDetial = $user->getInfo([
                'uid' => $this->uid
            ], 'user_tel');
            if ($userDetial['user_tel']) {
                $employeeModel = new BcEmployeeModel();
                $employeeList  = $employeeModel->getQuery([], '*', 'id');
                $employeeTel   = [];
                foreach ($employeeList as $v) {
                    $employeeTel[] = $v['mobile'];
                }
                if (in_array($userDetial['user_tel'], $employeeTel)) {
                    return 1;
                } else {
                    return 0;
                }
     ...
     * (non-PHPdoc)
     *
     * @see \data\api\IMember::addMemberLocalAddress()
     */
    public function addMemberLocalAddress($consigner, $mobile, $phone, $province, $city, $district, $address, $zip_code, $alias)
    {
        $express_address = new NsMemberExpressAddressModel();
    ...
    public function getMemberViewHistory()
    {
        ...
        if ($has_history) {
            ...
            $list           = $goods->getGoodsQueryLimit([
                ...
                'ng.state'    => 1
            ], "ng.goods_id,ng.goods_name,ng_sap.pic_cover_mid,ng.price");
            return $list;
        } else {
            return '';
        }
    ...
    public function addMemberLevel($shop_id, $level_name, $goods_discount, $desc)
    {
        $member_level = new NsMemberLevelModel();
        $data         = array(
            'shop_id'        => $shop_id,
    ...
    public function updateMemberLevel($level_id, $shop_id, $level_name, $goods_discount, $desc)
    {
        $member_level     = new NsMemberLevelModel();
        $data             = array(
            'shop_id'        => $shop_id,
...
//        }
        ...
        if (!empty($list['data'])) {
            foreach ($list['data'] as $k => $v) {
                $record_account              = new BcDistributorAccountRecordsModel();
                $info                        = $record_account->getInfo([
                    'data_id' => $v['id']
                ]);
                $list['data'][$k]['account'] = $info["balance_record"] + $info["bonus_record"];
            }
        }
        return $list;
    }
        ...
        if ($member_balance_withdraw_info["transfer_status"] != 1 && $member_balance_withdraw_info["status"] != 1 && $status != -1) {
            if ($transfer_type == 1) {
                ...
                if ($type_id == 1) {
                    ...
                    } else if ($alipay_retval["result_code"] == "FAIL") {
                        $transfer_status = -1;
    ...
    public function setKol($uid, $kol_name, $kol_code, $memo, $status)
    {
        $member = new NsMemberModel();
        $res    = $member->save([
            'is_kol' => 1
        ...
        if ($res) {
            ...
            $retval = \think\Db::name('bc_kol')->insert($data);
        }
        return $retval;
    }

    public function setDistribution($uid, $distributor_type, $name, $store_id)
    {
        $retval = parent::updateUserRealName($uid, $name);
        if ($retval < 0) {
            return $retval;
        } else {
            // 分销设置
            $member = new NsMemberModel();
            $retval = $member->save(['real_name' => $name, 'distributor_type' => $distributor_type, 'source_branch' => $store_id], ['uid' => $uid]);
            if ($retval < 0) {
                return $retval;
            } else {
                $data        = array(
                ...
                $info        = $distributor->getInfo(['uid' => $uid]);
                if ($info) {
                    $retval = $distributor->save($data, ['uid' => $uid]);
                } else {
                    $distributor->save($data);
                }
                return $retval;
            }
