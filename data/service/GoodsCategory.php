...
class GoodsCategory extends BaseService implements IGoodsCategory
{
     ...
     */
    public function addOrEditGoodsCategory($category_id, $category_name, $short_name, $pid, $is_visible, $keywords = '', $description = '', $sort = 0, $category_pic, $category_ads, $attr_id = 0, $attr_name = '')
    {
    ...
    public function getGoodsCategoryFirst()
    {
        $cache = Cache::tag("niu_goods_category_first")->get("getGoodsCategoryFirst");
        if (empty($cache)) {
            $goods_category       = new NsGoodsCategoryModel();
            $goods_category_first = $goods_category->getQuery([
                "pid" => 0,
                "is_visible" => 1
            ], "*", "sort asc");
            Cache::tag("niu_goods_category_first")->set("getGoodsCategoryFirst", $goods_category_first);
            return $goods_category_first;
        } else {
            return $cache;
        }
    }
    ...
    public function getGoodsCategoryBlockQuery($shop_id, $show_num = 4)
    {
        ...
        if (empty($cache)) {
            ...
            foreach ($goods_category_block_list as $k => $v) {
                ...
                $goods_list = $goods->getGoodsQueryLimit($condition, "ng.goods_id,ng_sap.pic_cover_small,pic_cover_big,ng.goods_name,ng.promotion_price,ng.vip_price,ng.state,ng.shipping_fee,ng.group_id_array,ng.material_code,ng.source_type", $show_num);
    ...
    //所有产品销量前四
    public function getGoodsFourQuery($shop_id, $show_num = 4, $order = "ng.sales desc")
    {
        $cache = Cache::tag("niu_goods_category_block")->get("getGoodsFourQuery" . $shop_id . '_' . $show_num);
        if (empty($cache)) {
            $goods      = new Goods();
            $condition  = [
                "ng.state" => 1
            ];
            $goods_list = $goods->getGoodsQueryLimit($condition, "ng.goods_id,ng_sap.pic_cover_small,pic_cover_big,ng.goods_name,ng.promotion_price,ng.vip_price,ng.state,ng.shipping_fee,ng.group_id_array,ng.material_black,ng.use_black,ng.effect_black,ng.selling_point,ng.sales,ng.material_code,ng.source_type", $show_num, $order);
            Cache::tag("niu_goods_category_block")->set("getGoodsFourQuery" . $shop_id . '_' . $show_num, $goods_list, 1800); // 缓存半小时
            return $goods_list;
        } else {
            return $cache;
        }
    ...
    //黑科技排行榜
    public function getGoodsTopQuery($shop_id)
    {
        $cache = Cache::tag("niu_goods_category_block")->get("getGoodsTopQuery" . $shop_id);
        if (empty($cache)) {
            $goods      = new Goods();
            $condition  = [
                "ng.state" => 1,
            ...
            $goods_list = $goods->getBlackTechGoodsQuery($condition, "ng.goods_id,ng_sap.pic_cover_small,pic_cover_big,ng.goods_name,ng.promotion_price,ng.vip_price,ng.state,ng.shipping_fee,ng.group_id_array,ng.material_black,ng.use_black,ng.effect_black,ng.selling_point,ng.sales,ng.source_type,ng.material_code");
            foreach ($goods_list as $key => $value) {
                $goods_list[$key]["all_black"] = round(($value['material_black'] + $value['use_black'] + $value['effect_black']) / 3, 1);
            }
            Cache::tag("niu_goods_category_block")->set("getGoodsTopQuery" . $shop_id, $goods_list, 1800); // 缓存半小时
            return $goods_list;
        } else {
            return $cache;
        }
    ...
    //黑科技体验专区
    public function getGoodsExperienceQuery($shop_id)
    {
        $cache = Cache::tag("niu_goods_category_block")->get("getGoodsExperienceQuery" . $shop_id);
        if (empty($cache)) {
            $goods      = new Goods();
            $condition  = [
                "ng.state" => 1,
            ...
            $goods_list = $goods->getBlackTechGoodsQuery($condition, "ng.goods_id,ng_sap.pic_cover_small,pic_cover_big,ng.goods_name,ng.promotion_price,ng.vip_price,ng.state,ng.shipping_fee,ng.group_id_array,ng.material_black,ng.use_black,ng.effect_black,ng.selling_point,ng.sales,ng.material_code,ng.source_type");
            foreach ($goods_list as $key => $value) {
                $goods_list[$key]["all_black"] = round(($value['material_black'] + $value['use_black'] + $value['effect_black']) / 3, 1);
            }
            Cache::tag("niu_goods_category_block")->set("getGoodsExperienceQuery" . $shop_id, $goods_list, 1800); // 缓存半小时
            return $goods_list;
        } else {
            return $cache;
        }
    ...
    //小样
    public function getGoodsSmallSampleQuery($shop_id)
    {
        $cache = Cache::tag("niu_goods_category_block")->get("getGoodsSmallSampleQuery" . $shop_id);
        if (empty($cache)) {
            $goods      = new Goods();
            $condition  = [
                "ng.state" => 1,
            ...
            $goods_list = $goods->getBlackTechGoodsQuery($condition, "ng.goods_id,ng_sap.pic_cover_small,ng.vip_price,pic_cover_big,ng.goods_name,ng.promotion_price,ng.state,ng.shipping_fee,ng.group_id_array,ng.sales,ng.source_type,ng.material_code");
            Cache::tag("niu_goods_category_block")->set("getGoodsSmallSampleQuery" . $shop_id, $goods_list, 1800); // 缓存半小时
            return $goods_list;
        } else {
            return $cache;
        }
    ...
    public function getCategoryTreeUseInShopIndex()
    {
        ...
        if (empty($cache)) {
            ...
            $condition['category_name'] = array(
                'neq',
                '健康'
            );

            $goods_category_one = $goods_category_model->getQuery($condition, 'category_id, category_name,short_name,pid,category_pic', 'sort');
    ...
    public function getCategoryTreeUseInAdmin()
    {
        ...
        if (empty($cache)) {
            ...
            $goods_category_one = $goods_category_model->getQuery([
                'level' => 1
            ], 'category_id, category_name,short_name,pid,category_pic,sort,attr_name,is_visible,user_click,click', 'sort');
            if (!empty($goods_category_one)) {
                foreach ($goods_category_one as $k_cat_one => $v_cat_one) {
                    $goods_category_two_list = $goods_category_model->getQuery([
                        'level' => 2,
    ...
    //首页底部商品列表
    public function getIndexGoodsQuery($shop_id, $show_num = 10, $order = "ng.sort asc")
    {
//        $cache = Cache::tag("niu_goods_category_block")->get("getGoodsFourQuery" . $shop_id . '_' . $show_num);
//        if (empty($cache)) {
        $goods      = new Goods();
        ...
        $goods_list = $goods->getGoodsQueryLimit($condition, "ng.goods_id,ng.sort,ng_sap.pic_cover_small,pic_cover_big,ng.goods_name,ng.promotion_price,ng.introduction,ng.vip_price,ng.state,ng.shipping_fee,ng.group_id_array,ng.material_black,ng.use_black,ng.effect_black,ng.selling_point,ng.sales,ng.img_id_array,ng.material_code,ng.source_type,ng.sale_type,ng.sale_end_time,ng.delivery_end_time", $show_num, $order);
        foreach ($goods_list as $key => $v) {
            $arr                              = explode(',', $v['img_id_array']);
            $info                             = \think\Db::name('sys_album_picture')->where(['pic_id' => $arr[count($arr) - 1]])->find();
            $goods_list[$key]['last_picture'] = $info['pic_cover_small'];

            // 当前用户是否收藏了该商品
            $goods_list[$key]['is_member_fav_goods'] = 0;
            if (!empty($this->uid)) {
                $member                                  = new Member();
                $goods_list[$key]['is_member_fav_goods'] = $member->getIsMemberFavorites($this->uid, $v['goods_id'], 'goods');
            }
        }
//            Cache::tag("niu_goods_category_block")->set("getGoodsFourQuery" . $shop_id . '_' . $show_num, $goods_list, 1800); // 缓存半小时
        return $goods_list;
...
//        }
    ...
    //首页底部商品列表
    public function getIndexGoodsQueryPage($page_index, $page_size, $order = "ng.sort asc")
    {
        $goods      = new Goods();
        $condition   = array(
            'ng.is_black_tech' => array(
        ...
        $goods_list = $goods->getGoodsQueryLimitPage($condition, "ng.goods_id,ng.sort,ng_sap.pic_cover_small,pic_cover_big,ng.goods_name,ng.promotion_price,ng.introduction,ng.vip_price,ng.state,ng.shipping_fee,ng.group_id_array,ng.material_black,ng.use_black,ng.effect_black,ng.selling_point,ng.sales,ng.img_id_array,ng.material_code,ng.source_type,ng.sale_type,ng.sale_end_time,ng.delivery_end_time", $page_index, $page_size, $order)['data'];
        foreach ($goods_list as $key => $v) {
            $arr                              = explode(',', $v['img_id_array']);
            $info                             = \think\Db::name('sys_album_picture')->where(['pic_id' => $arr[count($arr) - 1]])->find();
            $goods_list[$key]['last_picture'] = $info['pic_cover_small'];
