...
class OrderGoods extends BaseService
{
     ...
     */
    public function addOrderGoods($order_id, $goods_sku_list, $is_inside = 0, $distributor_type = 0, $parent_source_distribution = 0, $gift_flag = 0, $adjust_money = 0)
    {
        ...
        try {
            ...
            foreach ($goods_sku_list_array as $k => $goods_sku_array) {
                ...
                $goods_sku_info = $goods_sku_model->getInfo([
                    'sku_id' => $goods_sku[0]
                ], 'sku_id,goods_id,cost_price,stock,sku_name,code,material_code,attr_value_items');
                ...
                ], 'goods_name,price,goods_type,picture,promotion_type,promote_id,point_exchange_type,give_point,fraction,sale_type,sale_end_time,delivery_end_time');
                ...
                $mansong_info = \think\Db::name('ns_promotion_mansong')->where(['is_neigou' => 1 , 'status' => 1])->find();

                if($mansong_info['mansong_id'] > 0){
                    $condition1['discount_id']                = $mansong_info['mansong_id'];
                    $condition1['goods_id']                   = $goods_sku_info['goods_id'];
                    $condition1['sku_id']                     = $goods_sku_info['sku_id'];
                    $promotion_goods_info                     = $NG_MODEL->getInfo($condition1);
                    //获取内购订单的商品内购价
                    if($promotion_goods_info){
                        $n_price = sprintf("%.2f", $promotion_goods_info['n_price'] == 0 ?
                            $goods_sku_info['price'] * $promotion_goods_info['n_discount'] /10 :
                            $promotion_goods_info['n_price']);
                        if($is_inside == 1 && $n_price > 0 && $n_price < $sku_price){
                            $sku_price = $n_price;
                        }
                    }
                }
                ...
                //判断是否分润
                if($is_inside == 0){
                    if($distributor_type == 4){
                        $fraction = $goods_info['fraction'];
                    }elseif($goods_info['fraction'] > 0  && $distributor_type == 2){
                        $fraction = 0.05;   //分润大于0的商品统一给有分润门店极选师0.05的分润
                    }else{
                        $fraction = 0;
                    }
                }else{
                ...
                //计算直接分润和间接分润
                if($parent_source_distribution == 0){
                    $direct_separation = floor(($sku_price * $goods_sku[1] - $adjust_money) * $fraction * 100) / 100;
                    $indirect_separation = 0;
                }else{
                    $direct_separation = floor(($sku_price * $goods_sku[1] - $adjust_money) * $fraction * 0.8 * 100) / 100;
                    $indirect_separation = floor(($sku_price * $goods_sku[1] - $adjust_money) * $fraction * 0.2 * 100) / 100;
                }
                ...
                $data_order_sku = array(
                    ...
                    'code' => $goods_sku_info['code'],
                    'material_code' => $goods_sku_info['material_code'],
                    'price' => $sku_price,
                    'num' => $goods_sku[1],
                    ...
                    'indirect_separation' => $indirect_separation,
                    'sale_type' => $goods_info['sale_type'],
                    'sale_end_time'=>$goods_info['sale_end_time'],
                    'delivery_end_time' => $goods_info['delivery_end_time']
                ); // 积分数量默认0
    ...
    public function addShareOrderGoods($order_id, $goods_sku_list, $distributor_type = 0, $parent_source_distribution = 0, $gift_flag = 0, $adjust_money = 0)
    {
        $this->order_goods->startTrans();
        try {
            $err = 0;
...
//                }
                ...
                //计算直接分润和间接分润
                if($parent_source_distribution == 0){
                    $direct_separation = floor($goods_sku[2] * $goods_sku[1] * $fraction * 100)/100;
                    $indirect_separation = 0;
                }else{
                    $direct_separation = floor($goods_sku[2] * $goods_sku[1] * $fraction * 0.8 * 100)/100;
                    $indirect_separation = floor($goods_sku[2] * $goods_sku[1] * $fraction * 0.2 * 100)/100;
                }
                ...
                $data_order_sku = array(
                    ...
                    'price' => $sku_price,
                    'num' => $goods_sku[1],
                    'adjust_money' => $goods_sku[2] * $goods_sku[1] - $sku_price * $goods_sku[1],
                    'cost_price' => $goods_sku_info['cost_price'],
                    'goods_money' => $goods_sku[2] * $goods_sku[1],
                    'goods_picture' => $picture != 0 ? $picture : $goods_info['picture'], // 如果当前商品有SKU图片，就用SKU图片。没有则用商品主图
                    'shop_id' => $this->instance_id,
                    'buyer_id' => $this->uid,
                    'goods_type' => $goods_info['goods_type'],
    ...
    public function addOrderGiftGoods($order_id, $goods_sku_list, $adjust_money = 0)
    {
        ...
        try {
            $err = 0;
//            $goods_sku_list_array = explode(",", $goods_sku_list);
            foreach ($goods_sku_list as $k => $goods_sku_array) {
                $goods_sku = explode(':', $goods_sku_array);
                $goods_sku_model = new NsGoodsSkuModel();
                $goods_sku_info = $goods_sku_model->getInfo([
                    'sku_id' => $goods_sku[0]
                ], 'sku_id,goods_id,cost_price,stock,sku_name,attr_value_items,code,material_code');
                ...
                //查询赠品id    todo  @戴  修改   goods_id   ->   sku_id
                $gift_goods_model = new NsPromotionGiftGoodsModel();
                $gift_goods_info = $gift_goods_model->getInfo(['sku_id'=>$goods_sku[0]],"gift_id");
    ...
    public function orderGoodsRefundAskfor($order_id, $order_goods_id, $refund_type, $refund_require_money, $refund_reason)
    {
        ...
        try {
            ...
            // 退款记录
            $id = $this->addOrderRefundAction($order_goods_id, $status_id, 1, $this->uid);
    ...
    public function orderGoodsRefundAskfors($order_id, $order_goods_id, $refund_type, $refund_require_money, $refund_require_num, $refund_reason)
    {
        ...
        try {
            ...
            $refund_records_id = $order->addOrderRefundAccountRecordses( $order_goods_id, $refund_type, $refund_require_money, $refund_require_num, $refund_reason, $status_id, $refund_trade_no);

            // 更新订单项
            $order_goods = new NsOrderGoodsModel();
            $order_goods_info = $order_goods->getInfo([
                'order_goods_id' => $order_goods_id
            ], 'refund_total_money,refund_total_num');
            $order_goods_data = array(
                'refund_total_money' => $order_goods_info['refund_total_money']+$refund_require_money,
                'refund_total_num' => $order_goods_info['refund_total_num']+$refund_require_num,
                'refund_status' => $status_id,
                'refund_records_id' => $refund_records_id
            );
            $res = $order_goods->save($order_goods_data, [
                'order_goods_id' => $order_goods_id
            ]);
            ...
            // 退款操作记录
            $id = $this->addOrderRefundActions($order_goods_id, $refund_records_id, $status_id, 1, $this->uid);

            // 主订单状态变更
            $order = new Order();
            $res = $order->orderGoodsRefundFinishes($order_id);
    ...
    public function giftOrderGoodsRefundAskfor($order_id, $order_goods_id, $refund_type, $refund_require_money, $refund_reason)
    {
        ...
        try {
            ...
            // 退款记录
            $id = $this->addGiftOrderRefundAction($order_goods_id, $status_id, 1);
    ...
    public function orderGoodsReturnGoodses($order_id, $order_goods_id, $refund_records_id, $refund_shipping_company, $refund_shipping_code)
    {
        ...
        try {
            ...
            $ret = $refundAccountRecordsmodel->save($data, [
                'id' => $refund_records_id
            ]);
    ...
    public function orderGoodsRefundAgree($order_id, $order_goods_id)
    {
        ...
        try {
            ...
            $id = $this->addOrderRefundAction($order_goods_id, $status_id, 2, $this->uid);
            $this->order_goods->commit();
            //推送
            if($id){
                $ret = order_refund_push(2,$id);
    ...
    public function orderGoodsRefundAgrees($order_id, $order_goods_id, $refund_records_id)
    {
        ...
        try {
            ...
            // 退款信息
            $refundAccountRecords = NsOrderRefundAccountRecordsModel::get($refund_records_id);
            $refund_type = $refundAccountRecords->refund_type;
            if ($refund_type == 1) { // 仅退款
                $status_id = OrderStatus::getRefundStatus()[3]['status_id'];
            } else { // 退货退款
                $status_id = OrderStatus::getRefundStatus()[1]['status_id'];
            ...
            // 添加退款日志
            $id = $this->addOrderRefundActions($order_goods_id, $refund_records_id, $status_id, 2, $this->uid);
    ...
    public function orderGoodsRefuseForevers($order_id, $order_goods_id, $refund_records_id)
    {
        ...
        try {
            ...
            $refund_account_record = $refundAccountRecordsmodel->getInfo([
                'id' => $refund_records_id
            ], 'refund_require_money,refund_require_num');
            $order_goods_data = array(
                'refund_total_money' => $order_goods_info['refund_total_money']-$refund_account_record['refund_require_money'],
                'refund_total_num' => $order_goods_info['refund_total_num']-$refund_account_record['refund_require_num'],
                'refund_status' => $status_id
            );
            $res = $order_goods->save($order_goods_data, [
                'order_goods_id' => $order_goods_id
    ...
    public function orderGoodsConfirmRefunds($order_id, $order_goods_id, $refund_records_id, $refund_real_money, $refund_require_num, $refund_balance_money, $refund_trade_no, $refund_way, $refund_remark)
    {
        ...
        try {
            ...
            if (empty($refund_remark)) {
                $remark = "订单编号:" . $order_info['order_no'] . "，退款方式为:[" . OrderStatus::getPayType($refund_way) . "]，退款金额:" . $refund_real_money . "元";
            } else {
                $remark = $refund_remark;
            }
            $data = array(
...
//            ], 'refund_total_money,refund_total_num');
//            $order_goods_data = array(
//                'refund_total_money' => $order_goods_info['refund_total_money']+$refund_real_money,
//                'refund_total_num' => $order_goods_info['refund_total_num']+$refund_require_num,
//                'refund_status' => $status_id
//            );
//            $res = $order_goods->save($order_goods_data, [
...
//            ]);
            ...
            $order_goods_info = $order_goods->getInfo([
                'order_goods_id' => $order_goods_id
            ], 'refund_total_money,fraction,direct_separation,indirect_separation');
            $refund_account_record = $refundAccountRecordsmodel->getInfo([
                'id' => $refund_records_id
            ], 'refund_require_money');
            if($order_goods_info['fraction'] > 0){
                if($order_info['parent_source_distribution'] == 0){
                    $direct_separation = floor(($order_goods_info['direct_separation'] - $refund_real_money*$order_goods_info['fraction'])*100)/100;
                    $indirect_separation = 0;
                }else{
                    $direct_separation = floor(($order_goods_info['direct_separation'] - $refund_real_money*$order_goods_info['fraction']*0.8)*100)/100;
                    $indirect_separation = floor(($order_goods_info['indirect_separation'] - $refund_real_money*$order_goods_info['fraction']*0.2)*100)/100;
                }

                if($direct_separation < 0){
                    $direct_separation = 0;
                }
                if($indirect_separation < 0){
                    $indirect_separation = 0;
                }
            ...
            $order_goods_data = array(
                'refund_total_money' => $order_goods_info['refund_total_money']-$refund_account_record['refund_require_money']+$refund_real_money,
                'refund_status' => $status_id,
                'direct_separation' => $direct_separation,
                'indirect_separation' => $indirect_separation
            );
            ...
            // 订单恢复正常操作
            $order = new Order();
            $retval = $order->orderGoodsRefundFinishes($order_id);
    ...
    public function addOrderRefundActions($order_goods_id, $refund_records_id, $refund_status_id, $action_way, $uid)
    {
        $refund_status = OrderStatus::getRefundStatus();
        $refund_status_name = '';
        foreach ($refund_status as $k => $v) {
    ...
    public function addGiftOrderRefundAction($order_goods_id, $refund_status_id, $action_way)
    {
        $refund_status = OrderStatus::getRefundStatus();
        foreach ($refund_status as $k => $v) {
            if ($v['status_id'] == $refund_status_id) {
    ...
    public function getOrderGoodsRefundDetails($order_goods_id)
    {
        ...
        //查询订单项退款操作状态
        if ($order_goods_info['refund_status'] != 0 && $order_goods_info['refund_records_id'] != 0) {
            $order_refund_status = OrderStatus::getRefundStatus();
            foreach ($order_refund_status as $k_status => $v_status) {
                ...
                //查询订单项退款记录
                $refundAccountRecordsModel = new NsOrderRefundAccountRecordsModel();
                $refund_account_records  = $refundAccountRecordsModel->getQuery(['order_goods_id'=>$order_goods_id], '*', 'askfor_time desc');

                foreach ($refund_account_records as $k => $v) {
                    //查询退款记录操作状态
                    foreach ($order_refund_status as $k_status => $v_status) {
                        if ($v_status['status_id'] == $v['refund_status']) {
                            $refund_account_records[$k]['refund_operation'] = $v_status['refund_operation'];
                            $refund_account_records[$k]['status_name'] = $v_status['status_name'];
                        }
                    }

                    //查询退款记录操作日志
                    $orderRefundModel = new NsOrderRefundModel();
                    $refund_account_records[$k]['refund_action']  = $orderRefundModel->getQuery(['refund_records_id'=>$v['id']], '*', 'action_time desc');
                }

                $order_goods_info['refund_account_records'] = $refund_account_records;
                $order_goods_info['refund_account_record']  = $refund_account_records[0];
            }
        } else {
            $order_goods_info['refund_operation'] = '';
            $order_goods_info['status_name'] = '';
            $order_goods_info['refund_account_records'] = '';
            $order_goods_info['refund_account_record'] = '';
        }


        //获取订单项实际可退款金额(实际支付金额)
        $real_goods_money = $this->orderGoodsRefundMoney($order_goods_id);
        $order_goods_info['refund_max_money'] = $real_goods_money-$order_goods_info['refund_total_money'];
        $order_goods_info['refund_max_num'] = $order_goods_info['num']-$order_goods_info['refund_total_num'];
        return $order_goods_info;
    }
    ...
    public function getOrderGoodsIdArray($order_id)
    {
        $order_model = new NsOrderGoodsModel();
        $order_goods_info = $order_model->getQuery([
            'order_id' => $order_id
        ], 'order_goods_id','order_goods_id');
        ...
        $order_goods_id_array = substr($order_goods_id_array,0,strlen($order_goods_id_array)-1);
